...
`acmeclaw.invoke --each`) without adding new AcmeClaw code per workflow.
...
This extension depends on AcmeClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AcmeClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.acmeclaw/extensions` as a standalone plugin directory.
