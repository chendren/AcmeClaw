...
AcmeClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
acmeclaw channels login --channel zalouser
...
```bash
acmeclaw message send --channel zalouser --target <threadId> --message "Hello from AcmeClaw!"
```
...
```bash
acmeclaw channels login --channel zalouser              # Login via QR
acmeclaw channels login --channel zalouser --account work
acmeclaw channels status --probe
acmeclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
acmeclaw directory self --channel zalouser
acmeclaw directory peers list --channel zalouser --query "name"
acmeclaw directory groups list --channel zalouser --query "work"
acmeclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
acmeclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
acmeclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `acmeclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
acmeclaw channels login --channel zalouser --account work
acmeclaw message send --channel zalouser --account work --target <id> --message "Hello"
