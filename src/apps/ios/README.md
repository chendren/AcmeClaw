...
This is an **alpha** iOS app that connects to an AcmeClaw Gateway as a `role: node`.
...
3. On iOS: AcmeClaw → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project AcmeClaw.xcodeproj -scheme AcmeClaw -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AcmeClawKit` contains the shared transport/types used by the iOS app.
