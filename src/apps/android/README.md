## AcmeClaw Node (Android) (internal)

Modern Android node app: connects to the **Gateway WebSocket** (`_acmeclaw-gw._tcp`) and exposes **Canvas + Chat + Camera**.
...
```bash
pnpm acmeclaw gateway --port 18789 --verbose
```
...
```bash
acmeclaw nodes pending
acmeclaw nodes approve <requestId>
```
