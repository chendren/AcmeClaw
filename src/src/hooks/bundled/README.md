...
This directory contains hooks that ship with AcmeClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.acmeclaw/workspace`)

**Enable**:

```bash
acmeclaw hooks enable session-memory
```
...
```bash
acmeclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.acmeclaw/logs/commands.log`

**Enable**:

```bash
acmeclaw hooks enable command-logger
```
...
homepage: https://docs.acmeclaw.ai/hooks#my-hook
metadata:
  { "acmeclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.acmeclaw/hooks/` (shared across workspaces)
...
```bash
acmeclaw hooks info session-memory
```

Check hook status:

```bash
acmeclaw hooks check
```
...
```bash
acmeclaw hooks enable session-memory
acmeclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.acmeclaw/acmeclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'acmeclaw.*gateway' && pnpm acmeclaw gateway`
3. Enable the hook: `acmeclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.acmeclaw.ai/hooks
