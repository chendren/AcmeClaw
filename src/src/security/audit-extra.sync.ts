...
export function collectOutboundRedactionFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  if (cfg.logging?.redactOutbound === "off") {
    findings.push({
      checkId: "outbound-redaction-disabled",
      severity: "critical",
      title: "Outbound message redaction is disabled",
      detail:
        'logging.redactOutbound is set to "off". Secrets (API keys, tokens, passwords) in agent responses will be sent unredacted to external channels.',
      remediation:
        'Set logging.redactOutbound to "messages" or remove the setting to use the default.',
    });
  }
  return findings;
}
