...
import { redactOutboundMessage } from "../../security/redact-outbound.js";
import { createSubsystemLogger } from "../../logging/subsystem.js";
import { throwIfAborted } from "./abort.js";
import { ackDelivery, enqueueDelivery, failDelivery } from "./delivery-queue.js";
import { normalizeReplyPayloadsForDelivery } from "./payloads.js";

const securityLog = createSubsystemLogger("security/outbound");
...
async function deliverOutboundPayloadsCore(params: {
  ...
  for (const payload of normalizedPayloads) {
    ...
    // Redact secrets before they reach any external channel or plugin hook.
    let redactedPayload = payload;
    if (payloadSummary.text) {
      const redactResult = redactOutboundMessage(payloadSummary.text);
      if (redactResult.redacted) {
        securityLog.warn(
          `redacted ${redactResult.count} secret(s) from outbound message to ${channel}`,
        );
        redactedPayload = { ...payload, text: redactResult.text };
        payloadSummary.text = redactResult.text;
      }
    }
    ...
    try {
      ...
      let effectivePayload = redactedPayload;
      if (hookRunner?.hasHooks("message_sending")) {
        try {
          ...
          if (sendingResult?.content != null) {
            effectivePayload = { ...redactedPayload, text: sendingResult.content };
