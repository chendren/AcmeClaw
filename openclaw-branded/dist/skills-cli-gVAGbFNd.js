...
function formatSkillsList(report, opts) {
	...
	if (skills.length === 0) return appendClawHubHint(opts.eligible ? `No eligible skills found. Run \`${formatCliCommand("acmeclaw skills list")}\` to see all skills.` : "No skills found.", opts.json);
...
function formatSkillInfo(report, skillName, opts) {
	...
	if (!skill) {
		...
		return appendClawHubHint(`Skill "${skillName}" not found. Run \`${formatCliCommand("acmeclaw skills list")}\` to see available skills.`, opts.json);
	}
