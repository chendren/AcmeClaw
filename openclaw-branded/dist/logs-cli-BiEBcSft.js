...
function createLogWriters() {
	const writer = createSafeStreamWriter({
		...
		onBrokenPipe: (err, stream) => {
			...
			const message = `acmeclaw logs: output ${stream === process.stdout ? "stdout" : "stderr"} closed (${code}). Stopping tail.`;
			try {
...
function emitGatewayError(err, opts, mode, rich, emitJsonLine, errorLine) {
	...
	const hint = `Hint: run \`${formatCliCommand("acmeclaw doctor")}\`.`;
