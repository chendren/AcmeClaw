...
async function completionCacheExists(shell, binName = "acmeclaw") {
...
function isCompletionProfileHeader(line) {
	return line.trim() === "# AcmeClaw Completion";
}
...
function updateCompletionProfile(content, binName, cachePath, sourceLine) {
	...
	const block = `# AcmeClaw Completion\n${sourceLine}`;
...
async function isCompletionInstalled(shell, binName = "acmeclaw") {
...
* Returns true if profile has `source <(acmeclaw completion ...)` instead of cached file.
*/
async function usesSlowDynamicCompletion(shell, binName = "acmeclaw") {
	const profilePath = getShellProfilePath(shell);
	if (!await pathExists(profilePath)) return false;
...
async function installCompletion(shell, yes, binName = "acmeclaw") {
	const home = process.env.HOME || os.homedir();
	let profilePath = "";
	let sourceLine = "";
