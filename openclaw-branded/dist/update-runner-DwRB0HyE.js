...
const PRIMARY_PACKAGE_NAME = "acmeclaw";
...
async function resolvePortableGitPathPrepend(env) {
	...
	const portableGitRoot = path.join(localAppData, "AcmeClaw", "deps", "portable-git");
	const candidates = [
...
function normalizeTag(tag) {
	return normalizePackageTagInput(tag, ["acmeclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
		...
		if (channel === "dev") {
			...
			const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "acmeclaw-update-preflight-"));
		...
		const doctorEntry = path.join(gitRoot, "acmeclaw.mjs");
		if (!await fs.stat(doctorEntry).then(() => true).catch(() => false)) {
			steps.push({
				name: "acmeclaw doctor entry",
		...
		const doctorStep = await runStep(step("acmeclaw doctor", [
