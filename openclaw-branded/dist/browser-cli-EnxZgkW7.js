...
import "./acmeclaw-root-3m-COvjr.js";
...
function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
	...
	browser.command("upload").description("Arm file upload for the next file chooser").argument("<paths...>", "File paths to upload (must be within AcmeClaw temp uploads dir, e.g. /tmp/acmeclaw/uploads/file.pdf)").option("--ref <ref>", "Ref id from snapshot to click after arming").option("--input-ref <ref>", "Ref id for <input type=file> to set directly").option("--element <selector>", "CSS selector for <input type=file>").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next file chooser (default: 120000)", (v) => Number(v)).action(async (paths, opts, cmd) => {
		const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
	...
	browser.command("waitfordownload").description("Wait for the next download (and save it)").argument("[path]", "Save path within acmeclaw temp downloads dir (default: /tmp/acmeclaw/downloads/...; fallback: os.tmpdir()/acmeclaw/downloads/...)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next download (default: 120000)", (v) => Number(v)).action(async (outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/wait/download",
			body: { path: outPath?.trim() || void 0 }
		});
	});
	browser.command("download").description("Click a ref and save the resulting download").argument("<ref>", "Ref id from snapshot to click").argument("<path>", "Save path within acmeclaw temp downloads dir (e.g. report.pdf or /tmp/acmeclaw/downloads/report.pdf)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the download to start (default: 120000)", (v) => Number(v)).action(async (ref, outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/download",
			body: {
				ref,
...
function registerBrowserDebugCommands(browser, parentOpts) {
	...
	trace.command("stop").description("Stop trace recording and write a .zip").option("--out <path>", "Output path within acmeclaw temp dir (e.g. trace.zip or /tmp/acmeclaw/trace.zip)").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (opts, cmd) => {
		await withDebugContext(cmd, parentOpts, async ({ parent, profile }) => {
			const result = await callDebugRequest(parent, {
				method: "POST",
				path: "/trace/stop",
...
const browserCoreExamples = [
	"acmeclaw browser status",
	"acmeclaw browser start",
	"acmeclaw browser stop",
	"acmeclaw browser tabs",
	"acmeclaw browser open https://example.com",
	"acmeclaw browser focus abcd1234",
	"acmeclaw browser close abcd1234",
	"acmeclaw browser screenshot",
	"acmeclaw browser screenshot --full-page",
	"acmeclaw browser screenshot --ref 12",
	"acmeclaw browser snapshot",
	"acmeclaw browser snapshot --format aria --limit 200",
	"acmeclaw browser snapshot --efficient",
	"acmeclaw browser snapshot --labels"
];
const browserActionExamples = [
	"acmeclaw browser navigate https://example.com",
	"acmeclaw browser resize 1280 720",
	"acmeclaw browser click 12 --double",
	"acmeclaw browser type 23 \"hello\" --submit",
	"acmeclaw browser press Enter",
	"acmeclaw browser hover 44",
	"acmeclaw browser drag 10 11",
	"acmeclaw browser select 9 OptionA OptionB",
	"acmeclaw browser upload /tmp/acmeclaw/uploads/file.pdf",
	"acmeclaw browser fill --fields '[{\"ref\":\"1\",\"value\":\"Ada\"}]'",
	"acmeclaw browser dialog --accept",
	"acmeclaw browser wait --text \"Done\"",
	"acmeclaw browser evaluate --fn '(el) => el.textContent' --ref 7",
	"acmeclaw browser console --level error",
	"acmeclaw browser pdf"
];
//#endregion
...
async function installChromeExtension(opts) {
	...
	if (!hasManifest(src)) throw new Error("Bundled Chrome extension is missing. Reinstall AcmeClaw and try again.");
...
function registerBrowserExtensionCommands(browser, parentOpts) {
	...
	ext.command("install").description("Install the Chrome extension to a stable local path").action(async (_opts, cmd) => {
		...
		defaultRuntime.error(info([
			...
			`- Pin “AcmeClaw Browser Relay”, then click it on the tab (badge shows ON)`,
			"",
			`${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.acmeclaw.ai/tools/chrome-extension")}`
		].join("\n")));
	});
	ext.command("path").description("Print the path to the installed Chrome extension (load unpacked)").action(async (_opts, cmd) => {
		...
		if (!hasManifest(dir)) {
			defaultRuntime.error(danger([`Chrome extension is not installed. Run: "${formatCliCommand("acmeclaw browser extension install")}"`, `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.acmeclaw.ai/tools/chrome-extension")}`].join("\n")));
			defaultRuntime.exit(1);
		}
		if (parent?.json) {
...
async function runBrowserToggle(parent, params) {
	...
	const name = status.profile ?? "acmeclaw";
...
function registerBrowserManageCommands(browser, parentOpts) {
	browser.command("status").description("Show browser status").action(async (_opts, cmd) => {
		...
		await runBrowserCommand$1(async () => {
			...
			defaultRuntime.log([
				`profile: ${status.profile ?? "acmeclaw"}`,
	...
	browser.command("profiles").description("List all browser profiles").action(async (_opts, cmd) => {
		...
		await runBrowserCommand$1(async () => {
			...
			defaultRuntime.log(profiles.map((p) => {
				...
				const driver = p.driver !== "acmeclaw" ? ` [${p.driver}]` : "";
				return `${p.name}: ${status}${tabs}${def}${remote}${driver}\n  ${loc}, color: ${p.color}`;
			}).join("\n"));
		});
	});
	browser.command("create-profile").description("Create a new browser profile").requiredOption("--name <name>", "Profile name (lowercase, numbers, hyphens)").option("--color <hex>", "Profile color (hex format, e.g. #0066CC)").option("--cdp-url <url>", "CDP URL for remote Chrome (http/https)").option("--driver <driver>", "Profile driver (acmeclaw|extension|existing-session). Default: acmeclaw").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		await runBrowserCommand$1(async () => {
			const result = await callBrowserRequest(parent, {
				method: "POST",
...
function registerBrowserCli(program) {
	const browser = program.command("browser").description("Manage AcmeClaw's dedicated browser (Chrome/Chromium)").option("--browser-profile <name>", "Browser profile name (default from config)").option("--json", "Output machine-readable JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([...browserCoreExamples, ...browserActionExamples].map((cmd) => [cmd, ""]), true)}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/browser", "docs.acmeclaw.ai/cli/browser")}\n`).action(() => {
		browser.outputHelp();
		defaultRuntime.error(danger(`Missing subcommand. Try: "${formatCliCommand("acmeclaw browser status")}"`));
		defaultRuntime.exit(1);
	});
