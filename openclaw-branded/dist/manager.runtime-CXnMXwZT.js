...
import { B as shouldLogVerbose, M as defaultRuntime, O as createSubsystemLogger, R as logVerbose, Z as resolvePreferredAcmeClawTmpDir } from "./logger-Cmh2bFJD.js";
...
function prependInternalEventContext(body, events) {
	if (body.includes("AcmeClaw runtime context (internal):")) return body;
...
async function prepareAgentCommandExecution(opts, runtime) {
	...
	if (agentIdOverride) {
		if (!listAgentIds(cfg).includes(agentIdOverride)) throw new Error(`Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("acmeclaw agents list")}" to see configured agents.`);
	}
...
async function writeWavFile(pcm) {
	const tempDir = await fs.mkdtemp(path.join(resolvePreferredAcmeClawTmpDir(), "discord-voice-"));
