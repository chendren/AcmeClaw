...
const DEFAULT_TAGLINE = "All your chats, one AcmeClaw.";
...
const TAGLINES = [
	...
	"You had me at 'acmeclaw gateway start.'",
...
const EXAMPLES = [
	["acmeclaw models --help", "Show detailed help for the models command."],
	["acmeclaw channels login --verbose", "Link personal WhatsApp Web and show QR + connection logs."],
	["acmeclaw message send --target +15555550123 --message \"Hi\" --json", "Send via your web session and print JSON result."],
	["acmeclaw gateway --port 18789", "Run the WebSocket Gateway locally."],
	["acmeclaw --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
	["acmeclaw gateway --force", "Kill anything bound to the default gateway port, then start it."],
	["acmeclaw gateway ...", "Gateway control via WebSocket."],
	["acmeclaw agent --to +15555550123 --message \"Run summary\" --deliver", "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply."],
	["acmeclaw message send --channel telegram --target @mychat --message \"Hi\"", "Send via your Telegram bot."]
];
function configureProgramHelp(program, ctx) {
	program.name(CLI_NAME).description("").version(ctx.programVersion).option("--dev", "Dev profile: isolate state under ~/.acmeclaw-dev, default gateway port 19001, and shift derived ports (browser/canvas)").option("--profile <name>", "Use a named profile (isolates OPENCLAW_STATE_DIR/OPENCLAW_CONFIG_PATH under ~/.acmeclaw-<name>)").option("--log-level <level>", `Global log level override for file + console (${CLI_LOG_LEVEL_VALUES})`, parseCliLogLevelOption);
	...
	if (hasFlag(process.argv, "-V") || hasFlag(process.argv, "--version") || hasRootVersionAlias(process.argv)) {
		...
		console.log(commit ? `AcmeClaw ${ctx.programVersion} (${commit})` : `AcmeClaw ${ctx.programVersion}`);
		process.exit(0);
	}
	...
	program.addHelpText("afterAll", ({ command }) => {
		...
		const docs = formatDocsLink("/cli", "docs.acmeclaw.ai/cli");
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
	...
	process$1.on("uncaughtException", (error) => {
		console.error("[acmeclaw] Uncaught exception:", formatUncaughtError(error));
		process$1.exit(1);
	});
	program.parseAsync(process$1.argv).catch((err) => {
		console.error("[acmeclaw] CLI failed:", formatUncaughtError(err));
		process$1.exit(1);
	});
}
//#endregion
