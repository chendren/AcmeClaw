...
var GatewayChatClient = class GatewayChatClient {
	constructor(connection) {
		...
		this.client = new GatewayClient({
			...
			clientDisplayName: "acmeclaw-tui",
...
function resolveGatewayDisconnectState(reason) {
	...
	if (/pairing required/i.test(reasonLabel)) return {
		...
		activityStatus: "pairing required: run acmeclaw devices list",
		pairingHint: "Pairing required. Run `acmeclaw devices list`, approve your request ID, then reconnect."
	};
	return {
		connectionStatus: `gateway disconnected: ${reasonLabel}`,
		activityStatus: "idle"
...
async function runTui(opts) {
	...
	const updateHeader = () => {
		...
		header.setText(theme.header(`acmeclaw tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`));
	};
