...
function stripUnknownConfigKeys(config) {
	const parsed = AcmeClawSchema.safeParse(config);
...
function detectEmptyAllowlistPolicy(cfg) {
	...
	const checkAccount = (account, prefix, parent, channelName) => {
		...
		if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) warnings.push(`- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("acmeclaw doctor --fix")}" to auto-migrate from pairing store when entries exist.`);
...
async function maybeMigrateLegacyConfig() {
	...
	const targetDir = path.join(home, ".acmeclaw");
	const targetPath = path.join(targetDir, "acmeclaw.json");
	try {
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (snapshot.legacyIssues.length > 0) {
		...
		} else fixHints.push(`Run "${formatCliCommand("acmeclaw doctor --fix")}" to apply compatibility migrations.`);
	}
	...
	if (normalized.changes.length > 0) {
		...
		else fixHints.push(`Run "${formatCliCommand("acmeclaw doctor --fix")}" to apply these changes.`);
	}
	...
	if (shouldRepair) {
		...
		if (hits.length > 0) note([`- Telegram allowFrom contains ${hits.length} non-numeric entries (e.g. ${hits[0]?.entry ?? "@"}); Telegram authorization requires numeric sender IDs.`, `- Run "${formatCliCommand("acmeclaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`].join("\n"), "Doctor warnings");
		const discordHits = scanDiscordNumericIdEntries(candidate);
		if (discordHits.length > 0) note([`- Discord allowlists contain ${discordHits.length} numeric entries (e.g. ${discordHits[0]?.path}=${discordHits[0]?.entry}).`, `- Discord IDs must be strings; run "${formatCliCommand("acmeclaw doctor --fix")}" to convert numeric IDs to quoted strings.`].join("\n"), "Doctor warnings");
		const allowFromScan = maybeRepairOpenPolicyAllowFrom(candidate);
		if (allowFromScan.changes.length > 0) note([...allowFromScan.changes, `- Run "${formatCliCommand("acmeclaw doctor --fix")}" to add missing allowFrom wildcards.`].join("\n"), "Doctor warnings");
		const emptyAllowlistWarnings = detectEmptyAllowlistPolicy(candidate);
		if (emptyAllowlistWarnings.length > 0) note(emptyAllowlistWarnings.join("\n"), "Doctor warnings");
		...
		if (toolsBySenderHits.length > 0) {
			...
			note([
				...
				`- Run "${formatCliCommand("acmeclaw doctor --fix")}" to migrate legacy keys to typed id: entries.`
			].join("\n"), "Doctor warnings");
		}
		...
		if (safeBinCoverage.length > 0) {
			...
			lines.push(`- Run "${formatCliCommand("acmeclaw doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`);
			note(lines.join("\n"), "Doctor warnings");
		}
	...
	if (unknown.removed.length > 0) {
		...
		if (shouldRepair) {
			...
			fixHints.push("Run \"acmeclaw doctor --fix\" to remove these keys.");
		}
	}
