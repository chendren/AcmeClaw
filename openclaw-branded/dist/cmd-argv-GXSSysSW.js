...
function classifyPortListener(listener, port) {
	...
	if (raw.includes("acmeclaw")) return "gateway";
	if (raw.includes("ssh")) {
...
function buildPortHints(listeners, port) {
	...
	const hints = [];
	if (kinds.has("gateway")) hints.push(`Gateway already running locally. Stop it (${formatCliCommand("acmeclaw gateway stop")}) or use a different port.`);
...
async function handlePortError(err, port, context, runtime = defaultRuntime) {
	if (err instanceof PortInUseError || isErrno(err) && err.code === "EADDRINUSE") {
		...
		if (details) {
			...
			if (/acmeclaw|src\/index\.ts|dist\/index\.js/.test(details)) runtime.error(warn("It looks like another AcmeClaw instance is already running. Stop it or pick a different port."));
		}
...
* returning control to the caller (typically `triggerAcmeClawRestart` →
...
/**
* Parse acmeclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout) {
	...
	for (const line of stdout.split(/\r?\n/).filter(Boolean)) if (line.startsWith("p")) {
		if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("acmeclaw")) pids.push(currentPid);
...
* Returns only PIDs that belong to acmeclaw gateway processes (not the current process).
*/
