...
const OPENCLAW_REPO_URL = "https://github.com/acmeclaw/acmeclaw.git";
const MAX_LOG_CHARS = 8e3;
const DEFAULT_PACKAGE_NAME = "acmeclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
function normalizeTag(value) {
	return normalizePackageTagInput(value, ["acmeclaw", DEFAULT_PACKAGE_NAME]);
}
...
async function resolveUpdateRoot() {
	return await resolveAcmeClawPackageRoot({
...
async function ensureGitCheckout(params) {
	...
	if (!await isGitCheckout(params.dir)) {
		if (!await isEmptyDir(params.dir)) throw new Error(`OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or an acmeclaw checkout.`);
...
async function tryWriteCompletionCache(root, jsonMode) {
	const binPath = path.join(root, "acmeclaw.mjs");
...
async function updateStatusCommand(opts) {
	...
	defaultRuntime.log(theme.heading("AcmeClaw update status"));
	defaultRuntime.log("");
...
const STEP_LABELS = {
	...
	"acmeclaw doctor entry": "Checking doctor entrypoint",
	"acmeclaw doctor": "Running doctor checks",
...
function inferUpdateFailureHints(result) {
	...
	if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
		...
		hints.push("Example: npm config set prefix ~/.local && npm i -g acmeclaw@latest");
	}
	if (failedStep.name.startsWith("global update") && (stderr.includes("node-gyp") || stderr.includes("prebuild"))) {
		...
		hints.push("If it still fails: npm i -g acmeclaw@latest --omit=optional");
	}
	return hints;
}
...
async function prepareRestartScript(env = process.env, gatewayPort = DEFAULT_GATEWAY_PORT) {
	...
	try {
		if (platform === "linux") {
			...
			filename = `acmeclaw-restart-${timestamp}.sh`;
...
`;
			...
			filename = `acmeclaw-restart-${timestamp}.bat`;
...
async function tryInstallShellCompletion(opts) {
	...
	if (!status.profileInstalled) {
		...
		if (isCancel(shouldInstall) || !shouldInstall) {
			if (!opts.skipPrompt) defaultRuntime.log(theme.muted(`Skipped. Run \`${replaceCliName(formatCliCommand("acmeclaw completion --install"), CLI_NAME)}\` later to enable.`));
			return;
		}
...
async function runPackageInstallUpdate(params) {
	...
	const packageName = (pkgRoot ? await readPackageName(pkgRoot) : await readPackageName(params.root)) ?? "acmeclaw";
...
async function maybeRestartService(params) {
		...
		try {
			...
			if (!params.opts.json && restartInitiated) {
				...
				if (health.healthy) defaultRuntime.log(theme.success("Daemon restart completed."));
					...
					defaultRuntime.log(theme.muted(`Run \`${replaceCliName(formatCliCommand("acmeclaw gateway status --deep"), CLI_NAME)}\` for details.`));
				}
				defaultRuntime.log("");
			}
		} catch (err) {
			if (!params.opts.json) {
				...
				defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${replaceCliName(formatCliCommand("acmeclaw gateway restart"), CLI_NAME)}`));
			}
		}
		return;
	}
	if (!params.opts.json) {
		defaultRuntime.log("");
		if (params.result.mode === "npm" || params.result.mode === "pnpm") defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("acmeclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("acmeclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
		else defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("acmeclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
	}
}
async function updateCommand(opts) {
	...
	if (opts.dryRun) {
		...
		else actions.push(`Run global package manager update with spec acmeclaw@${tag}`);
	...
	if (!opts.json) {
		defaultRuntime.log(theme.heading("Updating AcmeClaw..."));
		defaultRuntime.log("");
	}
	...
	if (result.status === "skipped") {
		...
		if (result.reason === "not-git-install") {
			defaultRuntime.log(theme.warn(`Skipped: this AcmeClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("acmeclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("acmeclaw gateway restart"), CLI_NAME)}\`.`));
			defaultRuntime.log(theme.muted(`Examples: \`${replaceCliName("npm i -g acmeclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g acmeclaw@latest", CLI_NAME)}\``));
		}
		defaultRuntime.exit(0);
		return;
	}
...
async function updateWizardCommand(opts = {}) {
	if (!process.stdin.isTTY) {
		defaultRuntime.error("Update wizard requires a TTY. Use `acmeclaw update --channel <stable|beta|dev>` instead.");
		defaultRuntime.exit(1);
		return;
	}
	const timeoutMs = parseTimeoutMsOrExit(opts.timeout);
		...
		if (!await isGitCheckout(gitDir)) {
			if (await pathExists(gitDir)) {
				if (!await isEmptyDir(gitDir)) {
					defaultRuntime.error(`OPENCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCLAW_GIT_DIR to an empty folder or an acmeclaw checkout.`);
					defaultRuntime.exit(1);
					return;
				}
			}
...
function registerUpdateCli(program) {
	const update = program.command("update").description("Update AcmeClaw and inspect update channel status").option("--json", "Output result as JSON", false).option("--no-restart", "Skip restarting the gateway service after a successful update").option("--dry-run", "Preview update actions without making changes", false).option("--channel <stable|beta|dev>", "Persist update channel (git + npm)").option("--tag <dist-tag|version>", "Override npm dist-tag or version for this update").option("--timeout <seconds>", "Timeout for each update step in seconds (default: 1200)").option("--yes", "Skip confirmation prompts (non-interactive)", false).addHelpText("after", () => {
		const fmtExamples = [
			["acmeclaw update", "Update a source checkout (git)"],
			["acmeclaw update --channel beta", "Switch to beta channel (git + npm)"],
			["acmeclaw update --channel dev", "Switch to dev channel (git + npm)"],
			["acmeclaw update --tag beta", "One-off update to a dist-tag or version"],
			["acmeclaw update --dry-run", "Preview actions without changing anything"],
			["acmeclaw update --no-restart", "Update without restarting the service"],
			["acmeclaw update --json", "Output result as JSON"],
			["acmeclaw update --yes", "Non-interactive (accept downgrade prompts)"],
			["acmeclaw update wizard", "Interactive update wizard"],
			["acmeclaw --update", "Shorthand for acmeclaw update"]
...
${theme.heading("Switch channels:")}
  ...
  - Run acmeclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.acmeclaw.ai/cli/update")}`;
	}).action(async (opts) => {
		try {
			await updateCommand({
	...
	update.command("status").description("Show update channel and version status").option("--json", "Output result as JSON", false).option("--timeout <seconds>", "Timeout for update checks in seconds (default: 3)").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["acmeclaw update status", "Show channel + version status."],
		["acmeclaw update status --json", "JSON output."],
		["acmeclaw update status --timeout 10", "Custom timeout."]
	])}\n\n${theme.heading("Notes:")}\n${theme.muted("- Shows current update channel (stable/beta/dev) and source")}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.acmeclaw.ai/cli/update")}`).action(async (opts, command) => {
		try {
