...
import { Aa as normalizeIMessageHandle, Cp as resolveDefaultSignalAccountId, Ia as normalizeDiscordSlug, Lt as resolveDiscordChannelAllowlist, Nu as createPluginLoaderLogger, Qa as isChannelConfigured, Sp as listSignalAccountIds, br as resolveDiscordUserAllowlist, ht as detectBinary, ot as resolveSlackChannelAllowlist, to as listChannelPluginCatalogEntries, wp as resolveSignalAccount, yr as resolveSlackUserAllowlist, z as loadAcmeClawPlugins } from "./auth-profiles-DRjqKE3G.js";
...
async function promptSingleChannelSecretInput(params) {
	if (await resolveSecretInputModeForEnvSelection({
		...
		copy: {
			...
			plaintextHint: "Stores the credential directly in AcmeClaw config",
...
const imessageOnboardingAdapter = {
	...
	configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
		...
		await prompter.note([
			...
			"Ensure AcmeClaw has Full Disk Access to Messages DB.",
...
async function installSignalCliFromRelease(runtime) {
	...
	const tmpDir = await fs$1.mkdtemp(path.join(os.tmpdir(), "acmeclaw-signal-"));
...
const signalOnboardingAdapter = {
	...
	configure: async ({ cfg, runtime, prompter, accountOverrides, shouldPromptAccountIds, options }) => {
		...
		await prompter.note([
			"Link device with: signal-cli link -n \"AcmeClaw\"",
			"Scan QR in Signal → Linked Devices",
			`Then run: ${formatCliCommand("acmeclaw gateway call channels.status --params '{\"probe\":true}'")}`,
			`Docs: ${formatDocsLink("/signal", "signal")}`
		].join("\n"), "Signal next steps");
		return {
			cfg: next,
...
function buildSlackManifest(botName) {
	const safeName = botName.trim() || "AcmeClaw";
	const manifest = {
		display_information: {
			name: safeName,
			description: `${safeName} connector for AcmeClaw`
		},
		features: {
			bot_user: {
			...
			slash_commands: [{
				command: "/acmeclaw",
				description: "Send a message to AcmeClaw",
				should_escape: false
			}]
		},
...
async function noteTelegramTokenHelp(prompter) {
	await prompter.note([
		...
		"Website: https://acmeclaw.ai"
	].join("\n"), "Telegram bot token");
}
async function noteTelegramUserIdHelp(prompter) {
	await prompter.note([
		`1) DM your bot, then read from.id in \`${formatCliCommand("acmeclaw logs --follow")}\` (safest)`,
...
async function promptWhatsAppOwnerAllowFrom(params) {
	...
	await prompter.note("We need the sender/owner number so AcmeClaw can allowlist you.", "WhatsApp number");
...
async function promptWhatsAppAllowFrom(cfg, _runtime, prompter, options) {
	...
	if (await prompter.select({
		...
		options: [{
			...
			value: "separate",
			label: "Separate phone just for AcmeClaw"
		}]
...
const BUILTIN_ONBOARDING_ADAPTERS = [
	telegramOnboardingAdapter,
	{
		...
		configure: async ({ cfg, runtime, prompter, options, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
			...
			else if (!linked) await prompter.note(`Run \`${formatCliCommand("acmeclaw channels login")}\` later to link WhatsApp.`, "WhatsApp");
...
async function noteChannelPrimer(prompter, channels) {
	...
	await prompter.note([
		...
		`Approve with: ${formatCliCommand("acmeclaw pairing approve <channel> <code>")}`,
		"Public DMs require dmPolicy=\"open\" + allowFrom=[\"*\"].",
		"Multi-user DMs: run: " + formatCliCommand("acmeclaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.",
...
async function maybeConfigureDmPolicies(params) {
	...
	const selectPolicy = async (policy) => {
		await prompter.note([
			...
			`Approve: ${formatCliCommand(`acmeclaw pairing approve ${policy.channel} <code>`)}`,
	...
	const ensureBundledPluginEnabled = async (channel) => {
		...
		if (!getChannelPlugin(channel)) {
			if (getChannelOnboardingAdapter(channel)) {
				await prompter.note(`${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand("acmeclaw plugins list")}\` and \`${formatCliCommand("acmeclaw plugins enable " + channel)}\`, then restart the gateway.`, "Channel setup");
	...
	if (options?.quickstartDefaults) {
		...
		const choice = await prompter.select({
			...
			options: [...buildSelectionOptions(entries), {
				value: "__skip__",
				label: "Skip for now",
				hint: `You can add channels later via \`${formatCliCommand("acmeclaw channels add")}\``
			}],
