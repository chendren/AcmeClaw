...
async function promptRemoteGatewayConfig(cfg, prompter, options) {
	...
	if (!hasBonjourTool) await prompter.note(["Bonjour discovery requires dns-sd (macOS) or avahi-browse (Linux).", "Docs: https://docs.acmeclaw.ai/gateway/discovery"].join("\n"), "Discovery");
	if (wantsDiscover) {
	...
	if (authChoice === "token") {
		if (await resolveSecretInputModeForEnvSelection({
			...
			copy: {
				...
				plaintextHint: "Stores the token directly in AcmeClaw config"
			}
		...
		if (await resolveSecretInputModeForEnvSelection({
			...
			copy: {
				...
				plaintextHint: "Stores the password directly in AcmeClaw config"
			}
