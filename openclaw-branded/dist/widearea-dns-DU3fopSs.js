...
function extractContentHash(zoneText) {
	return zoneText.match(/^\s*;\s*acmeclaw-content-hash:\s*(\S+)\s*$/m)?.[1] ?? null;
}
...
function renderZone(opts) {
	const hostname = os.hostname().split(".")[0] ?? "acmeclaw";
	const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "acmeclaw");
	const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "acmeclaw-gw");
	...
	records.push(`_acmeclaw-gw._tcp IN PTR ${instanceLabel}._acmeclaw-gw._tcp`);
	records.push(`${instanceLabel}._acmeclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
	records.push(`${instanceLabel}._acmeclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
	const contentBody = `${records.join("\n")}\n`;
	return `; acmeclaw-content-hash: ${computeContentHash(`${records.map((line) => line === soaLine ? `@ IN SOA ns1 hostmaster SERIAL 7200 3600 1209600 60` : line).join("\n")}\n`)}\n${contentBody}`;
}
