...
async function removeChannelConfigWizard(cfg, runtime) {
	...
	while (true) {
		...
		if (configured.length === 0) {
			note$1(["No channel config found in acmeclaw.json.", `Tip: \`${formatCliCommand("acmeclaw channels status")}\` shows what is configured and enabled.`].join("\n"), "Remove channel");
			return next;
		}
...
async function promptGatewayConfig(cfg, runtime) {
	...
	if (authMode === "token") if (guardCancel(await select$1({
		...
		note$1(`Validated ${envVarName}. AcmeClaw will store a token SecretRef.`, "Gateway token");
	} else {
	...
	if (authMode === "trusted-proxy") {
		note$1([
			"Trusted proxy mode: AcmeClaw trusts user identity from a reverse proxy.",
			...
			"Docs: https://docs.acmeclaw.ai/gateway/trusted-proxy-auth"
		].join("\n"), "Trusted Proxy Auth");
...
async function promptChannelMode(runtime) {
	return guardCancel(await select$1({
		...
		options: [{
			...
			hint: "Delete channel tokens/settings from acmeclaw.json"
		}],
...
async function runConfigureWizard(opts, runtime = defaultRuntime) {
	try {
		...
		intro$1(opts.command === "update" ? "AcmeClaw update wizard" : "AcmeClaw configure");
		...
		if (snapshot.exists) {
			...
			if (!snapshot.valid && snapshot.issues.length > 0) note$1([
				...
				"",
				"Docs: https://docs.acmeclaw.ai/gateway/configuration"
			].join("\n"), "Config issues");
			if (!snapshot.valid) {
				outro$1(`Config invalid. Run \`${formatCliCommand("acmeclaw doctor")}\` to repair it, then re-run configure.`);
				runtime.exit(1);
				return;
			}
		}
