...
async function packNpmSpecToArchive(params) {
	...
	if (res.code !== 0) {
		...
		if (/E404|is not in this registry/i.test(raw)) return {
			ok: false,
			error: `Package not found on npm: ${params.spec}. See https://docs.acmeclaw.ai/tools/plugin for installable plugins.`
		};
		return {
			ok: false,
...
async function installPackageDir(params) {
	...
	try {
		...
		stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".acmeclaw-install-stage-"));
	...
	if (params.mode === "update" && await fileExists(canonicalTargetDir)) {
		const backupRoot = path.join(installBaseRealPath, ".acmeclaw-install-backups");
