...
function registerDirectoryCli(program) {
	const directory = program.command("directory").description("Lookup contact and group IDs (self, peers, groups) for supported chat channels").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["acmeclaw directory self --channel slack", "Show the connected account identity."],
		["acmeclaw directory peers list --channel slack --query \"alice\"", "Search contact/user IDs by name."],
		["acmeclaw directory groups list --channel discord", "List available groups/channels."],
		["acmeclaw directory groups members --channel discord --group-id <id>", "List members for a specific group."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/directory", "docs.acmeclaw.ai/cli/directory")}\n`).action(() => {
