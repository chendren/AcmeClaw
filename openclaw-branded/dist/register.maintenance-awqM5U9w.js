...
async function dashboardCommand(runtime = defaultRuntime, options = {}) {
	...
	if (resolvedToken.unresolvedRefReason) {
		...
		runtime.log("Set OPENCLAW_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `acmeclaw dashboard`.");
	}
	...
	if (opened) runtime.log("Opened in your browser. Keep that tab to control AcmeClaw.");
...
function logBackupRecommendation$1(runtime) {
	runtime.log(`Recommended first: ${formatCliCommand("acmeclaw backup create")}`);
}
async function resetCommand(runtime, opts) {
	...
	if (scope === "config+creds+sessions") {
		...
		runtime.log(`Next: ${formatCliCommand("acmeclaw onboard --install-daemon")}`);
		return;
	}
	if (scope === "full") {
...
async function removeMacApp(runtime, dryRun) {
	...
	await removePath("/Applications/AcmeClaw.app", runtime, {
		dryRun,
		label: "/Applications/AcmeClaw.app"
	});
}
		...
		const selection = await multiselectStyled({
			...
			options: [
				...
				},
				{
					value: "app",
					label: "macOS app",
					hint: "/Applications/AcmeClaw.app"
				}
			],
			initialValues: [
				"service",
