...
function formatAmbiguousGatewayAuthModeReason() {
	return ["gateway.auth.token and gateway.auth.password are both configured while gateway.auth.mode is unset.", `Set ${formatCliCommand("acmeclaw config set gateway.auth.mode token")} or ${formatCliCommand("acmeclaw config set gateway.auth.mode password")}.`].join(" ");
}
...
function scheduleDetachedLaunchdRestartHandoff(params) {
	...
	try {
		const child = spawn("/bin/sh", [
			...
			"acmeclaw-launchd-restart-handoff",
...
function resolveGatewayStateDir(env) {
	...
	return path.join(home, `.acmeclaw${suffix}`);
}
//#endregion
...
function throwBootstrapGuiSessionError(params) {
	throw new Error([
		...
		"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.acmeclaw.ai/gateway"
	].join("\n"));
}
...
async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		serviceTarget: `${domain}/${label}`,
		plistPath,
		actionHint: "acmeclaw gateway install --force"
	});
...
async function restartLaunchAgent({ stdout, env }) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		serviceTarget,
		plistPath,
		actionHint: "acmeclaw gateway restart"
	});
...
function isGatewayArgv(args, opts) {
	...
	return exe.endsWith("/acmeclaw") || exe === "acmeclaw" || opts?.allowGatewayBinary === true && exe.endsWith("/acmeclaw-gateway");
}
//#endregion
...
function buildSystemdUnit({ description, programArguments, workingDirectory, environment }) {
	...
	const descriptionValue = description?.trim() || "AcmeClaw Gateway";
...
function renderGatewayServiceStartHints(env = process.env) {
	...
	return buildPlatformServiceStartHints({
		installCommand: formatCliCommand("acmeclaw gateway install", env),
		startCommand: formatCliCommand("acmeclaw gateway", env),
	...
	if (loaded) {
		if (!opts.force) {
			...
			if (!json) {
				...
				defaultRuntime.log(`Reinstall with: ${formatCliCommand("acmeclaw gateway install --force")}`);
			}
			return;
		}
	}
...
function auditGatewayToken(command, issues, expectedGatewayToken) {
	...
	issues.push({
		...
		detail: "Run `acmeclaw gateway install --force` to remove embedded service token.",
		level: "recommended"
	});
	...
	issues.push({
		...
		message: "Gateway service OPENCLAW_GATEWAY_TOKEN does not match gateway.auth.token in acmeclaw.json",
...
function checkTokenDrift(params) {
	...
	if (configToken && serviceToken !== configToken) return {
		...
		detail: "Run `acmeclaw gateway install --force` to sync the token.",
		level: "recommended"
	};
	return null;
}
...
function renderSystemdUnavailableHints(options = {}) {
	...
	return ["systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.", `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("acmeclaw gateway")}\`.`];
}
//#endregion
...
async function runServiceStart(params) {
	...
	}
	{
		...
		if (configError) {
			fail(`${params.serviceNoun} aborted: config is invalid.\n${configError}\nFix the config and retry, or run "acmeclaw doctor" to repair.`);
			return;
		}
	}
	try {
...
async function restartGatewayWithoutServiceManager(port) {
	...
	const pids = resolveVerifiedGatewayListenerPids(port);
	if (pids.length === 0) return null;
	if (pids.length > 1) throw new Error(`multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "acmeclaw gateway status --deep" before retrying restart`);
	...
	return await runServiceRestart({
		...
		postRestartCheck: async ({ warnings, fail, stdout }) => {
			if (restartedWithoutServiceManager) {
				...
				fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [formatCliCommand("acmeclaw gateway status --deep"), formatCliCommand("acmeclaw doctor")]);
			}
...
function hasGatewayServiceMarker(content) {
	const lower = content.toLowerCase();
	const markerKeys = ["acmeclaw_service_marker"];
	const kindKeys = ["acmeclaw_service_kind"];
...
function isAcmeClawGatewayLaunchdService(label, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!contents.toLowerCase().includes("gateway")) return false;
	return label.startsWith("ai.acmeclaw.");
}
function isAcmeClawGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("acmeclaw-gateway")) return false;
	return contents.toLowerCase().includes("gateway");
}
function isAcmeClawGatewayTaskName(name) {
	const normalized = name.trim().toLowerCase();
	if (!normalized) return false;
	return normalized === resolveGatewayWindowsTaskName().toLowerCase() || normalized.startsWith("acmeclaw gateway");
}
...
async function scanLaunchdDir(params) {
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		...
		if (marker === "acmeclaw" && isAcmeClawGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
			detail: `plist: ${fullPath}`,
			scope: params.scope,
			marker,
			legacy: marker !== "acmeclaw" || isLegacyLabel(label)
		});
	}
	return results;
}
async function scanSystemdDir(params) {
	...
	for (const { entry, name, fullPath, contents } of candidates) {
		const marker = detectMarker(contents);
		if (!marker) continue;
		if (marker === "acmeclaw" && isAcmeClawGatewaySystemdService(name, contents)) continue;
		results.push({
			platform: "linux",
			label: entry,
...
async function findExtraGatewayServices(env, opts = {}) {
	...
	if (process.platform === "win32") {
		...
		for (const task of tasks) {
			...
			if (!name) continue;
			if (isAcmeClawGatewayTaskName(name)) continue;
...
function printDaemonStatus(status, opts) {
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("acmeclaw doctor")}" (or "${formatCliCommand("acmeclaw doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("acmeclaw gateway install --force")}\` from the same --profile / OPENCLAW_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (service.runtime?.cachedLabel) {
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("acmeclaw gateway install")}`));
		spacer();
	}
	...
	defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("acmeclaw status")}`);
	defaultRuntime.log(`${label("Troubleshooting:")} https://docs.acmeclaw.ai/troubleshooting`);
}
//#endregion
