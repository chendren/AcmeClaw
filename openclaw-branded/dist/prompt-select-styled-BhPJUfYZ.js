...
async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
	...
	if (deprecated.has("anthropic:claude-cli")) lines.push(`- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("acmeclaw models auth setup-token")}`);
	if (deprecated.has("openai-codex:codex-cli")) lines.push(`- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth → ${formatCliCommand("acmeclaw models auth login --provider openai-codex")}`);
	note$1(lines.join("\n"), "Auth profiles");
...
function formatAuthIssueHint(issue) {
	...
	if (issue.provider === "anthropic" && issue.profileId === "anthropic:claude-cli") return `Deprecated profile. Use ${formatCliCommand("acmeclaw models auth setup-token")} or ${formatCliCommand("acmeclaw configure")}.`;
	if (issue.provider === "openai-codex" && issue.profileId === "openai-codex:codex-cli") return `Deprecated profile. Use ${formatCliCommand("acmeclaw models auth login --provider openai-codex")} or ${formatCliCommand("acmeclaw configure")}.`;
	return `Re-auth via \`${formatCliCommand("acmeclaw configure")}\` or \`${formatCliCommand("acmeclaw onboard")}\`.`;
}
...
async function maybeRepairLegacyCronStore(params) {
	...
	note$1([
		...
		...previewLines,
		`Repair with ${formatCliCommand("acmeclaw doctor --fix")} to normalize the store before the next scheduler run.`
	].join("\n"), "Cron");
...
function buildGatewayRuntimeHints(runtime, options = {}) {
	...
	if (runtime.cachedLabel && platform === "darwin") {
		...
		hints.push(`Then reinstall: ${formatCliCommand("acmeclaw gateway install", env)}`);
	}
	if (runtime.missingUnit) {
		hints.push(`Service not installed. Run: ${formatCliCommand("acmeclaw gateway install", env)}`);
		if (fileLog) hints.push(`File logs: ${fileLog}`);
		return hints;
	}
...
async function maybeRepairGatewayDaemon(params) {
	...
	if (process.platform === "darwin") {
		...
		note$1(`LaunchAgent loaded; stopping requires "${formatCliCommand("acmeclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`, "Gateway");
	}
	if (serviceRuntime?.status === "running") {
		if (await params.prompter.confirmSkipInNonInteractive({
...
async function maybeScanExtraGatewayServices(options, runtime, prompter) {
	...
	if (legacyServices.length > 0) {
		if (await prompter.confirmSkipInNonInteractive({
			...
			if (removed.length > 0) runtime.log("Legacy gateway services removed. Installing AcmeClaw gateway next.");
		}
	}
...
* Runs as part of `acmeclaw doctor` — config-only, no network calls.
*/
	...
	if (resolved.provider !== "auto") {
		if (resolved.provider === "local") {
			if (hasLocalEmbeddings(resolved.local, true)) {
				if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
					...
					note$1([
						...
						"",
						`Verify: ${formatCliCommand("acmeclaw memory status --deep")}`
					].filter(Boolean).join("\n"), "Memory search");
				}
				return;
			}
			note$1([
				...
				`- Switch to a remote provider: ${formatCliCommand("acmeclaw config set agents.defaults.memorySearch.provider openai")}`,
				"",
				`Verify: ${formatCliCommand("acmeclaw memory status --deep")}`
			].join("\n"), "Memory search");
			return;
		...
		note$1([
			...
			`- Configure credentials: ${formatCliCommand("acmeclaw configure --section model")}`,
			`- To disable: ${formatCliCommand("acmeclaw config set agents.defaults.memorySearch.enabled false")}`,
			"",
			`Verify: ${formatCliCommand("acmeclaw memory status --deep")}`
		].join("\n"), "Memory search");
		return;
...
async function noteMacLaunchAgentOverrides() {
	...
	const markerPath = [path.join(home, ".acmeclaw", "disable-launchagent")].find((candidate) => fs.existsSync(candidate));
...
function noteStartupOptimizationHints(env = process.env, deps) {
	...
	const suggestions = [
		...
		"  export NODE_COMPILE_CACHE=/var/tmp/acmeclaw-compile-cache",
		"  mkdir -p /var/tmp/acmeclaw-compile-cache",
...
async function maybeRepairSandboxImages(cfg, runtime, prompter) {
	...
	if (!await isDockerAvailable()) {
		note$1([
			...
			"- Disable sandbox mode: acmeclaw config set agents.defaults.sandbox.mode off"
		].join("\n"), "Sandbox");
		return cfg;
	}
	let next = cfg;
	...
	await handleMissingSandboxImage({
		kind: "base",
		image: dockerImage,
		buildScript: dockerImage === "acmeclaw-sandbox-common:bookworm-slim" ? "scripts/sandbox-common-setup.sh" : dockerImage === "acmeclaw-sandbox:bookworm-slim" ? "scripts/sandbox-setup.sh" : void 0,
		updateConfig: (image) => {
...
async function noteSecurityWarnings(cfg) {
	const warnings = [];
	const auditHint = `- Run: ${formatCliCommand("acmeclaw security audit --deep")}`;
	if (cfg.approvals?.exec?.enabled === false) warnings.push("- Note: approvals.exec.enabled=false disables approval forwarding only.", "  Host exec gating still comes from ~/.acmeclaw/exec-approvals.json.", `  Check local policy with: ${formatCliCommand("acmeclaw approvals get --gateway")}`);
	...
	const saferRemoteAccessLines = [
		...
		"  Docs: https://docs.acmeclaw.ai/gateway/remote"
	];
	if (isExposed) if (!hasSharedSecret) {
		const authFixLines = resolvedAuth.mode === "password" ? [`  Fix: ${formatCliCommand("acmeclaw configure")} to set a password`, `  Or switch to token: ${formatCliCommand("acmeclaw config set gateway.auth.mode token")}`] : [`  Fix: ${formatCliCommand("acmeclaw doctor --fix")} to generate a token`, `  Or set token directly: ${formatCliCommand("acmeclaw config set gateway.auth.mode token")}`];
		warnings.push(`- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`, `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`, `  Fix: ${formatCliCommand("acmeclaw config set gateway.bind loopback")}`, ...saferRemoteAccessLines, ...authFixLines);
	...
	const warnDmPolicy = async (params) => {
		...
		if (dmScope === "main" && isMultiUserDm) warnings.push(`- ${params.label} DMs: multiple senders share the main session; run: ` + formatCliCommand("acmeclaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.");
	};
...
async function noteSessionLockHealth(params) {
	...
	if (staleCount > 0 && !shouldRepair) {
		...
		lines.push("- Run \"acmeclaw doctor --fix\" to remove stale lock files automatically.");
	}
...
function findOtherStateDirs(stateDir) {
	...
	for (const root of roots) {
		...
		for (const entry of entries) {
			...
			const candidates = [".acmeclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
async function noteStateIntegrity(cfg, prompter, configPath) {
	...
	const defaultStateDir = path.join(homedir(), ".acmeclaw");
	...
	if (cloudSyncedStateDir) warnings.push([
		...
		"- Prefer a local non-synced state dir (for example: ~/.acmeclaw).",
		`  Set locally: OPENCLAW_STATE_DIR=~/.acmeclaw ${formatCliCommand("acmeclaw doctor")}`
	].join("\n"));
	...
	if (entries.length > 0) {
		...
		if (missing.length > 0) warnings.push([
			...
			`  Verify sessions in store: ${formatCliCommand(`acmeclaw sessions --store "${absoluteStorePath}"`)}`,
			`  Preview cleanup impact: ${formatCliCommand(`acmeclaw sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
			`  Prune missing entries: ${formatCliCommand(`acmeclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`
		].join("\n"));
...
function noteWorkspaceBackupTip(workspaceDir) {
	...
	note$1([
		...
		"- Keep ~/.acmeclaw out of git; it contains credentials and session history.",
...
async function detectAcmeClawGitCheckout(root) {
...
async function maybeOfferUpdateBeforeDoctor(params) {
	...
	const git = await detectAcmeClawGitCheckout(params.root);
	if (git === "git") {
		if (!await params.confirm({
			message: "Update AcmeClaw from git before running doctor?",
			initialValue: true
	...
	if (git === "not-git") note$1(["This install is not a git checkout.", `Run \`${formatCliCommand("acmeclaw update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`].join("\n"), "Update");
	return { updated: false };
}
//#endregion
...
const MEMORY_SYSTEM_PROMPT = [
	...
	"https://github.com/acmeclaw/acmeclaw/commit/9ffea23f31ca1df5183b25668f8f814bee0fb34e",
	"https://github.com/acmeclaw/acmeclaw/commit/7d1fee70e76f2f634f1b41fca927ee663914183a"
].join("\n");
...
async function doctorCommand(runtime = defaultRuntime, options = {}) {
	...
	if (!cfg.gateway?.mode) {
		const lines = [
			...
			`Fix: run ${formatCliCommand("acmeclaw configure")} and set Gateway mode (local/remote).`,
			`Or set directly: ${formatCliCommand("acmeclaw config set gateway.mode local")}`
		];
		if (!fs.existsSync(configPath)) lines.push(`Missing config: run ${formatCliCommand("acmeclaw setup")} first.`);
		note$1(lines.join("\n"), "Gateway");
	}
	if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) note$1([
		"gateway.auth.token and gateway.auth.password are both configured while gateway.auth.mode is unset.",
		"Set an explicit mode to avoid ambiguous auth selection and startup/runtime failures.",
		`Set token mode: ${formatCliCommand("acmeclaw config set gateway.auth.mode token")}`,
		`Set password mode: ${formatCliCommand("acmeclaw config set gateway.auth.mode password")}`
	].join("\n"), "Gateway auth");
	...
	} else if (!prompter.shouldRepair) runtime.log(`Run "${formatCliCommand("acmeclaw doctor --fix")}" to apply changes.`);
