import { t as createSubsystemLogger, z as resolvePreferredAcmeClawTmpDir } from "./subsystem-D2xHvZZd.js";
import { d as isRecord, g as resolveConfigDir, t as CONFIG_DIR, y as resolveUserPath } from "./utils-DwZbjiC4.js";
import { _ as normalizeSkillFilter, a as openBoundaryFileSync, n as resolveAcmeClawPackageRootSync, p as isPathInside$1 } from "./acmeclaw-root-3m-COvjr.js";
import { d as resolveAcmeClawMetadata, f as resolveSkillInvocationPolicy, l as shouldIncludeSkill, u as parseFrontmatter } from "./env-overrides-DfvB37sI.js";
...
const PLUGIN_MANIFEST_FILENAME = "acmeclaw.plugin.json";
...
function resolvePluginSourceRoots(params) {
	...
	return {
		...
		workspace: workspaceRoot ? path.join(workspaceRoot, ".acmeclaw", "extensions") : void 0
	};
}
...
function discoverAcmeClawPlugins(params) {
	const env = params.env ?? process.env;
...
async function resolveAllowedTmpMediaPath(params) {
	...
	const openClawTmpDir = path.resolve(resolvePreferredAcmeClawTmpDir());
...
function resolveBundledSkillsDir(opts = {}) {
	...
	try {
		...
		const packageRoot = resolveAcmeClawPackageRootSync({
...
function loadSkillEntries(workspaceDir, opts) {
	...
	const bundledSkills = bundledSkillsDir ? loadSkills({
		...
		source: "acmeclaw-bundled"
	}) : [];
	const extraSkills = mergedExtraDirs.flatMap((dir) => {
		return loadSkills({
			dir: resolveUserPath(dir),
			source: "acmeclaw-extra"
		});
	});
	const managedSkills = loadSkills({
		dir: managedSkillsDir,
		source: "acmeclaw-managed"
	});
	...
	return Array.from(merged.values()).map((skill) => {
		...
		return {
			skill,
			frontmatter,
			metadata: resolveAcmeClawMetadata(frontmatter),
...
function resolveWorkspaceSkillPromptState(workspaceDir, opts) {
	...
	return {
		...
		prompt: [
			remoteNote,
			truncated ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`acmeclaw skills check\` to audit.` : "",
