...
function resolveRequestedLoginProviderOrThrow(providers, rawProvider) {
	...
	throw new Error(`Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("acmeclaw plugins list --json")}\`.`);
}
...
async function modelsAuthLoginCommand(opts, runtime) {
	...
	if (providers.length === 0) throw new Error(`No provider plugins found. Install one via \`${formatCliCommand("acmeclaw plugins install")}\`.`);
...
async function loadModelRegistry(_cfg, _opts) {
	const agentDir = resolveAcmeClawAgentDir();
...
async function modelsListCommand(opts, runtime) {
	...
	const { ensureAcmeClawModelsJson } = await import("./reply-Bm8VrLQh.js").then((n) => n.pf);
	...
	let availabilityErrorMessage;
	try {
		await ensureAcmeClawModelsJson(sourceConfig ?? cfg);
...
async function modelsStatusCommand(opts, runtime) {
	...
	const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAcmeClawAgentDir();
	...
	if (missingProvidersInUse.length > 0) {
		...
		for (const provider of missingProvidersInUse) {
			const hint = provider === "anthropic" ? `Run \`claude setup-token\`, then \`${formatCliCommand("acmeclaw models auth setup-token")}\` or \`${formatCliCommand("acmeclaw configure")}\`.` : `Run \`${formatCliCommand("acmeclaw configure")}\` or set an API key env var.`;
