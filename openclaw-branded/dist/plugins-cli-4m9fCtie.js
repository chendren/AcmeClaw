...
function registerPluginsCli(program) {
	const plugins = program.command("plugins").description("Manage AcmeClaw plugins and extensions").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.acmeclaw.ai/cli/plugins")}\n`);
	...
	plugins.command("doctor").description("Report plugin load issues").action(() => {
		...
		const docs = formatDocsLink("/plugin", "docs.acmeclaw.ai/plugin");
		lines.push("");
