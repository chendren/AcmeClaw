...
async function appendStatusAllDiagnosis(params) {
	...
	lines.push("Troubleshooting: https://docs.acmeclaw.ai/troubleshooting");
	lines.push("");
}
//#endregion
...
async function buildStatusAllReportLines(params) {
	...
	const lines = [];
	lines.push(heading("AcmeClaw status --all"));
	lines.push("");
...
async function readServiceStatusSummary(service, fallbackLabel) {
	try {
		...
		const managedByAcmeClaw = command != null;
		const externallyManaged = !managedByAcmeClaw && runtime?.status === "running";
		const installed = managedByAcmeClaw || externallyManaged;
...
async function statusAllCommand(runtime, opts) {
	await withProgress({
		...
		const update = await checkUpdateStatus({
			root: await resolveAcmeClawPackageRoot({
		...
		const readServiceSummary = async (service) => {
			try {
				...
				return {
					...
					managedByAcmeClaw: summary.managedByAcmeClaw,
		...
		const lines = await buildStatusAllReportLines({
			...
			overviewRows: [
				...
				{
					Item: "Security",
					Value: `Run: ${formatCliCommand("acmeclaw security audit --deep")}`
				},
				gatewaySelfLine ? {
					Item: "Gateway self",
				...
				daemon ? {
					...
					Value: !daemon.installed ? `${daemon.label} not installed` : `${daemon.label} ${daemon.managedByAcmeClaw ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`
				} : {
					Item: "Gateway service",
					Value: "unknown"
				},
				nodeService ? {
					Item: "Node service",
					Value: !nodeService.installed ? `${nodeService.label} not installed` : `${nodeService.label} ${nodeService.managedByAcmeClaw ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`
				} : {
					Item: "Node service",
					Value: "unknown"
				},
...
async function statusCommand(opts, runtime) {
	...
	const daemonValue = (() => {
		...
		const installedPrefix = daemon.managedByAcmeClaw ? "installed · " : "";
		return `${daemon.label} ${installedPrefix}${daemon.loadedText}${daemon.runtimeShort ? ` · ${daemon.runtimeShort}` : ""}`;
	})();
	const nodeDaemonValue = (() => {
		if (nodeDaemon.installed === false) return `${nodeDaemon.label} not installed`;
		const installedPrefix = nodeDaemon.managedByAcmeClaw ? "installed · " : "";
	...
	runtime.log(theme.heading("AcmeClaw status"));
	runtime.log("");
	...
	if (pairingRecovery) {
		...
		if (pairingRecovery.requestId) runtime.log(theme.muted(`Recovery: ${formatCliCommand(`acmeclaw devices approve ${pairingRecovery.requestId}`)}`));
		runtime.log(theme.muted(`Fallback: ${formatCliCommand("acmeclaw devices approve --latest")}`));
		runtime.log(theme.muted(`Inspect: ${formatCliCommand("acmeclaw devices list")}`));
	}
	runtime.log("");
	...
	runtime.log(theme.muted(`Full report: ${formatCliCommand("acmeclaw security audit")}`));
	runtime.log(theme.muted(`Deep probe: ${formatCliCommand("acmeclaw security audit --deep")}`));
	runtime.log("");
	...
	runtime.log("");
	runtime.log("FAQ: https://docs.acmeclaw.ai/faq");
	runtime.log("Troubleshooting: https://docs.acmeclaw.ai/troubleshooting");
	runtime.log("");
	...
	runtime.log(`  Need to share?      ${formatCliCommand("acmeclaw status --all")}`);
	runtime.log(`  Need to debug live? ${formatCliCommand("acmeclaw logs --follow")}`);
	if (gatewayReachable) runtime.log(`  Need to test channels? ${formatCliCommand("acmeclaw status --deep")}`);
	else runtime.log(`  Fix reachability first: ${formatCliCommand("acmeclaw gateway probe")}`);
}
//#endregion
