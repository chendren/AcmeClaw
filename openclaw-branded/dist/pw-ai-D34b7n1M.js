...
import { z as resolvePreferredAcmeClawTmpDir } from "./subsystem-D2xHvZZd.js";
...
const OPENCLAW_EXTENSION_RELAY_BROWSER = "AcmeClaw/extension-relay";
...
function buildSiblingTempPath(targetPath) {
	...
	return path.join(path.dirname(targetPath), `.acmeclaw-output-${id}-${safeTail}.part`);
}
...
function buildTempDownloadPath(fileName) {
	...
	return path.join(resolvePreferredAcmeClawTmpDir(), "downloads", `${id}-${safeName}`);
}
...
async function screenshotWithLabelsViaPlaywright(opts) {
	...
	try {
		if (boxes.length > 0) await page.evaluate((labels) => {
			document.querySelectorAll("[data-acmeclaw-labels]").forEach((el) => el.remove());
			const root = document.createElement("div");
			root.setAttribute("data-acmeclaw-labels", "1");
			...
			for (const label of labels) {
				...
				box.setAttribute("data-acmeclaw-labels", "1");
				...
				tag.setAttribute("data-acmeclaw-labels", "1");
...
async function responseBodyViaPlaywright(opts) {
	...
	const resp = await new Promise((resolve, reject) => {
		...
		timer = setTimeout(() => {
			...
			cleanup();
			reject(/* @__PURE__ */ new Error(`Response not found for url pattern "${pattern}". Run '${formatCliCommand("acmeclaw browser requests")}' to inspect recent network activity.`));
		}, timeout);
	});
	const url = resp.url?.() || "";
