...
async function createAcpClient(opts = {}) {
	...
	const defaultServerCommand = entryPath ? process.execPath : "acmeclaw";
...
async function runAcpClientInteractive(opts = {}) {
	...
	console.log("AcmeClaw ACP client");
	...
	return {
		configOptions: [
			buildSelectConfigOption({
				...
				description: "Controls how much deliberate reasoning AcmeClaw requests from the Gateway model.",
			...
			buildSelectConfigOption({
				...
				name: "Tool verbosity",
				description: "Controls how much tool progress and output detail AcmeClaw keeps enabled for the session.",
			...
			buildSelectConfigOption({
				...
				name: "Usage detail",
				description: "Controls how much usage information AcmeClaw attaches to responses for the session.",
...
var AcpGatewayAgent = class {
	...
	assertSupportedSessionSetup(mcpServers) {
		...
		throw new Error("ACP bridge mode does not support per-session MCP servers. Configure MCP on the AcmeClaw gateway or agent instead.");
	}
...
function printHelp() {
	console.log(`Usage: acmeclaw acp [options]
...
function registerAcpCli(program) {
	...
	acp.command("client").description("Run an interactive ACP client against the local ACP bridge").option("--cwd <dir>", "Working directory for the ACP session").option("--server <command>", "ACP server command (default: acmeclaw)").option("--server-args <args...>", "Extra arguments for the ACP server").option("--server-verbose", "Enable verbose logging on the ACP server", false).option("-v, --verbose", "Verbose client logging", false).action(async (opts, command) => {
