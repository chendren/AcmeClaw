...
const NEW_STATE_DIRNAME = ".acmeclaw";
const CONFIG_FILENAME = "acmeclaw.json";
...
* Default: ~/.acmeclaw/acmeclaw.json (or $OPENCLAW_STATE_DIR/acmeclaw.json)
*/
...
function resolveDefaultConfigCandidates(env = process.env, homedir = envHomedir(env)) {
	...
	const candidates = [];
	const acmeclawStateDir = env.OPENCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
	if (acmeclawStateDir) {
		const resolved = resolveUserPath(acmeclawStateDir, env, effectiveHomedir);
...
* Default: os.tmpdir()/acmeclaw-<uid> (uid suffix when available).
*/
function resolveGatewayLockDir(tmpdir = os.tmpdir) {
	const base = tmpdir();
	const uid = typeof process.getuid === "function" ? process.getuid() : void 0;
	const suffix = uid != null ? `acmeclaw-${uid}` : "acmeclaw";
