...
async function promptSecretRefForOnboarding(params) {
	...
	while (true) {
		...
		if (source === "env") {
			...
			await params.prompter.note(params.copy?.envValidatedMessage?.(envVar) ?? `Validated environment variable ${envVar}. AcmeClaw will store a reference, not the key value.`, "Reference validated");
			return {
				ref,
				resolvedValue
			};
		...
		try {
			...
			await params.prompter.note(params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ?? `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. AcmeClaw will store a reference, not the key value.`, "Reference validated");
			return {
				ref,
				resolvedValue
			};
...
async function resolveSecretInputModeForEnvSelection(params) {
	...
	return await params.prompter.select({
		...
		options: [{
			...
			hint: params.copy?.plaintextHint ?? "Stores the key directly in AcmeClaw config"
		}, {
			value: "ref",
