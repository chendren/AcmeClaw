...
const DEFAULT_CLI_NAME = "acmeclaw";
const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE$1 = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(acmeclaw)\b/;
...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+acmeclaw\b|^acmeclaw\b/;
...
function formatPairingApproveHint(channelId) {
	return `Approve via: ${formatCliCommand(`acmeclaw pairing list ${channelId}`)} / ${formatCliCommand(`acmeclaw pairing approve ${channelId} <code>`)}`;
}
...
async function pickWebChannel(pref, authDir = resolveDefaultWebAuthDir()) {
	...
	if (!await webAuthExists(authDir)) throw new Error(`No WhatsApp Web session found. Run \`${formatCliCommand("acmeclaw channels login --channel whatsapp --verbose")}\` to link.`);
	return choice;
}
