...
import "./tmp-acmeclaw-dir-DEAexD45.js";
	...
	*/
	...
	async ensureMcporterDaemonStarted(mcporter) {
		...
		if (!mcporter.startDaemon) {
			const g = globalThis;
			if (!g.__acmeclawMcporterColdStartWarned) {
				g.__acmeclawMcporterColdStartWarned = true;
		...
		const g = globalThis;
		if (!g.__acmeclawMcporterDaemonStart) g.__acmeclawMcporterDaemonStart = (async () => {
			try {
				await this.runMcporter(["daemon", "start"], { timeoutMs: 1e4 });
			} catch (err) {
				log.warn(`mcporter daemon start failed: ${String(err)}`);
				delete g.__acmeclawMcporterDaemonStart;
			}
		})();
		await g.__acmeclawMcporterDaemonStart;
	}
