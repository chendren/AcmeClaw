...
import { n as resolveAcmeClawPackageRootSync, t as resolveAcmeClawPackageRoot } from "./acmeclaw-root-3m-COvjr.js";
	...
	for (const startDir of fallbackStartDirs) {
		...
		for (let i = 0; i < 8; i++) {
			...
			if (fs.existsSync(pkgJsonPath)) try {
				...
				if (JSON.parse(raw).name === "acmeclaw") return fs.existsSync(indexPath) ? indexPath : null;
				break;
			} catch {
				break;
			}
