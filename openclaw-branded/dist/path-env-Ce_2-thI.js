...
function candidateBinDirs(opts) {
	...
	try {
		...
		if (isExecutable(path.join(execDir, "acmeclaw"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "acmeclaw"))) append.push(localBinDir);
	}
...
/**
* Best-effort PATH bootstrap so skills that require the `acmeclaw` CLI can run
* under launchd/minimal environments (and inside the macOS app bundle).
*/
function ensureAcmeClawCliOnPath(opts = {}) {
...
//#endregion
export { ensureAcmeClawCliOnPath as t };
