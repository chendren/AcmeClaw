...
import { C as resolvePackageExtensionEntries, S as loadPluginManifest, _ as discoverAcmeClawPlugins } from "./skills-CtzUimzY.js";
...
const MISSING_EXTENSIONS_ERROR = "package.json missing acmeclaw.extensions; update the plugin package to include acmeclaw.extensions (for example [\"./dist/index.js\"]). See https://docs.acmeclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-acmeclaw-extensions";
const PLUGIN_INSTALL_ERROR_CODE = {
	INVALID_NPM_SPEC: "invalid_npm_spec",
	MISSING_OPENCLAW_EXTENSIONS: "missing_acmeclaw_extensions",
	EMPTY_OPENCLAW_EXTENSIONS: "empty_acmeclaw_extensions",
...
function ensureAcmeClawExtensions(params) {
	...
	if (resolved.status === "empty") return {
		ok: false,
		error: "package.json acmeclaw.extensions is empty",
...
async function installPluginFromPackageDir(params) {
	...
	const extensionsResult = ensureAcmeClawExtensions({ manifest });
	...
	try {
		...
		} else if (scanSummary.warn > 0) logger.warn?.(`Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "acmeclaw security audit --deep" for details.`);
	} catch (err) {
		logger.warn?.(`Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "acmeclaw security audit --deep" after install.`);
	}
...
async function installPluginFromNpmSpec(params) {
	...
	const finalized = finalizeNpmSpecArchiveInstall(await installFromNpmSpecArchiveWithInstaller({
		tempDirPrefix: "acmeclaw-npm-pack-",
		spec,
		timeoutMs,
...
function resolveBundledPluginSources(params) {
	const discovery = discoverAcmeClawPlugins({
