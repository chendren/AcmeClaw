...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (opts.installDaemon) {
		...
		if (!daemonInstall.installed && !opts.skipHealth) {
			logNonInteractiveOnboardingFailure({
				...
				hints: daemonInstall.skippedReason === "systemd-user-unavailable" ? ["Fix: rerun without `--install-daemon` for one-shot setup, or enable a working user-systemd session and retry.", "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `acmeclaw gateway run` or `acmeclaw agent --local`."] : [`Run \`${formatCliCommand("acmeclaw gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
		...
		if (!probe.ok) {
			...
			logNonInteractiveOnboardingFailure({
				...
				hints: !opts.installDaemon ? [
					...
					`Fix: start \`${formatCliCommand("acmeclaw gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
				...
				].filter((value) => Boolean(value)) : [`Run \`${formatCliCommand("acmeclaw gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("acmeclaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.acmeclaw.ai/tools/web`);
}
//#endregion
...
async function runNonInteractiveOnboardingRemote(params) {
	...
	if (opts.json) runtime.log(JSON.stringify(payload, null, 2));
		...
		runtime.log(`Tip: run \`${formatCliCommand("acmeclaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.acmeclaw.ai/tools/web`);
	}
}
//#endregion
...
async function runNonInteractiveOnboarding(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("acmeclaw doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
...
async function onboardCommand(opts, runtime = defaultRuntime) {
	...
	if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
		runtime.error([
			...
			"Read: https://docs.acmeclaw.ai/security",
			`Re-run with: ${formatCliCommand("acmeclaw onboard --non-interactive --accept-risk ...")}`
		].join("\n"));
		runtime.exit(1);
		return;
	}
	...
	if (process.platform === "win32") runtime.log([
		"Windows detected - AcmeClaw runs great on WSL2!",
		"Native Windows might be trickier.",
		"Quick setup: wsl --install (one command, one reboot)",
		"Guide: https://docs.acmeclaw.ai/windows"
	].join("\n"));
