...
async function requireRiskAcknowledgement(params) {
	...
	await params.prompter.note([
		...
		"",
		"AcmeClaw is a hobby project and still in beta. Expect sharp edges.",
		"By default, AcmeClaw is a personal agent: one trusted operator boundary.",
		"This bot can read files and run actions if tools are enabled.",
		"A bad prompt can trick it into doing unsafe things.",
		"",
		"AcmeClaw is not a hostile multi-tenant boundary by default.",
		"If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
		"",
		"If you’re not comfortable with security hardening and access control, don’t run AcmeClaw.",
		...
		"",
		"Run regularly:",
		"acmeclaw security audit --deep",
		"acmeclaw security audit --fix",
		"",
		"Must read: https://docs.acmeclaw.ai/gateway/security"
	].join("\n"), "Security");
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	...
	await prompter.intro("AcmeClaw onboarding");
	...
	if (snapshot.exists && !snapshot.valid) {
		...
		await prompter.outro(`Config invalid. Run \`${formatCliCommand("acmeclaw doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
	const quickstartHint = `Configure details later via ${formatCliCommand("acmeclaw configure")}.`;
