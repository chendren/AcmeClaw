...
function resolveProfileStateDir(profile, env, homedir) {
	...
	return path.join(resolveRequiredHomeDir(env, homedir), `.acmeclaw${suffix}`);
}
function applyCliProfileEnv(params) {
	...
	if (!env.OPENCLAW_CONFIG_PATH?.trim()) env.OPENCLAW_CONFIG_PATH = path.join(stateDir, "acmeclaw.json");
...
function ensureAcmeClawExecMarkerOnProcess(env = process.env) {
...
const ENTRY_WRAPPER_PAIRS = [{
	wrapperBasename: "acmeclaw.mjs",
	entryBasename: "entry.js"
}, {
	wrapperBasename: "acmeclaw.js",
	entryBasename: "entry.js"
}];
...
if (!isMainModule({
	...
	process$1.title = "acmeclaw";
	ensureAcmeClawExecMarkerOnProcess();
	...
	function ensureExperimentalWarningSuppressed() {
		...
		child.once("error", (error) => {
			console.error("[acmeclaw] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exit(1);
		});
		return true;
	}
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-DcA9ITyc.js").then((n) => n.r), import("./git-commit-B0vIx0co.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
			...
			console.log(commit ? `AcmeClaw ${VERSION} (${commit})` : `AcmeClaw ${VERSION}`);
			process$1.exit(0);
		}).catch((error) => {
			console.error("[acmeclaw] Failed to resolve version:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-DdPIyEwa.js").then(({ buildProgram }) => {
			...
			console.error("[acmeclaw] Failed to display help:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!parsed.ok) {
			console.error(`[acmeclaw] ${parsed.error}`);
			process$1.exit(2);
		}
		if (parsed.profile) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-Cu0soMcD.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
			console.error("[acmeclaw] Failed to start CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
	}
}
//#endregion
export { logAcceptedEnvOption as a, applyCliProfileEnv as c, isTruthyEnvValue as i, parseCliProfileArgs as l, markAcmeClawExecEnv as n, normalizeEnv as o, isMainModule as r, normalizeWindowsArgv as s, installProcessWarningFilter as t };
