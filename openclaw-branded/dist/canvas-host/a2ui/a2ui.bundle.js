...
var AcmeClawA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--acmeclaw-a2ui-inset-top, 0px)
        var(--acmeclaw-a2ui-inset-right, 0px)
        var(--acmeclaw-a2ui-inset-bottom, 0px)
        var(--acmeclaw-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--acmeclaw-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--acmeclaw-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--acmeclaw-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--acmeclaw-a2ui-empty-top, var(--acmeclaw-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		globalThis.acmeclawA2UI = api;
		this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
		this.#statusListener = (evt) => this.#handleActionStatus(evt);
		for (const eventName of ["acmeclaw:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["acmeclaw:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__acmeclawLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.acmeclawCanvasA2UIAction ?? globalThis.acmeclawCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.acmeclawCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("acmeclaw-a2ui-host")) customElements.define("acmeclaw-a2ui-host", AcmeClawA2UIHost);
