...
import { t as markAcmeClawExecEnv } from "./acmeclaw-exec-env-BIs3Hzjh.js";
import { c as normalizeResolvedSecretInputString } from "./types.secrets-ZqKzSyNC.js";
import { a as parseAcmeClawManifestInstallBase, c as resolveAcmeClawManifestOs, d as evaluateRuntimeEligibility, f as hasBinary, i as parseFrontmatterBool, l as resolveAcmeClawManifestRequires, n as getFrontmatterString, o as resolveAcmeClawManifestBlock, p as isConfigPathTruthyWithDefaults, r as normalizeStringList, s as resolveAcmeClawManifestInstall, t as applyAcmeClawManifestInstallCommonFields, u as parseFrontmatterBlock } from "./frontmatter-7FVJq8_7.js";
...
function sanitizeHostExecEnv(params) {
	...
	if (!overrides) return markAcmeClawExecEnv(merged);
	...
	return markAcmeClawExecEnv(merged);
}
...
function parseInstallSpec(input) {
	const parsed = parseAcmeClawManifestInstallBase(input, [
		"brew",
		"node",
		"go",
		"uv",
	...
	if (!parsed) return;
	const { raw } = parsed;
	const spec = applyAcmeClawManifestInstallCommonFields({ kind: parsed.kind }, parsed);
...
function resolveAcmeClawMetadata(frontmatter) {
	const metadataObj = resolveAcmeClawManifestBlock({ frontmatter });
	if (!metadataObj) return;
	const requires = resolveAcmeClawManifestRequires(metadataObj);
	const install = resolveAcmeClawManifestInstall(metadataObj, parseInstallSpec);
	const osRaw = resolveAcmeClawManifestOs(metadataObj);
	return {
...
const BUNDLED_SOURCES = new Set(["acmeclaw-bundled"]);
...
* @see https://github.com/acmeclaw/acmeclaw/issues/36280
*/
