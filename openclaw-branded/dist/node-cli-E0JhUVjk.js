...
import { s as sameFileIdentity } from "./acmeclaw-root-3m-COvjr.js";
...
function ensureNodePathEnv() {
	ensureAcmeClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
function renderNodeServiceStartHints() {
	return buildPlatformServiceStartHints({
		installCommand: formatCliCommand("acmeclaw node install"),
		startCommand: formatCliCommand("acmeclaw node start"),
...
async function runNodeDaemonInstall(opts) {
	...
	if (loaded && !opts.force) {
		...
		if (!json) {
			...
			defaultRuntime.log(`Reinstall with: ${formatCliCommand("acmeclaw node install --force")}`);
		}
		return;
	}
...
function registerNodeCli(program) {
	const node = program.command("node").description("Run and manage the headless node host service").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["acmeclaw node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
		["acmeclaw node status", "Check node host service status."],
		["acmeclaw node install", "Install the node host service."],
		["acmeclaw node restart", "Restart the installed node host service."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.acmeclaw.ai/cli/node")}\n`);
