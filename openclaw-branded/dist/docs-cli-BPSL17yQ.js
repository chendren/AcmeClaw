...
const SEARCH_TOOL = "https://docs.acmeclaw.ai/mcp.SearchAcmeClaw";
...
async function docsSearchCommand(queryParts, runtime) {
	...
	if (!query) {
		const docs = formatDocsLink("/", "docs.acmeclaw.ai");
		if (isRich()) {
			runtime.log(`${theme.muted("Docs:")} ${docs}`);
			runtime.log(`${theme.muted("Search:")} ${formatCliCommand("acmeclaw docs \"your query\"")}`);
		} else {
			runtime.log("Docs: https://docs.acmeclaw.ai/");
			runtime.log(`Search: ${formatCliCommand("acmeclaw docs \"your query\"")}`);
		}
		return;
	}
...
function registerDocsCli(program) {
	program.command("docs").description("Search the live AcmeClaw docs").argument("[query...]", "Search query").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/docs", "docs.acmeclaw.ai/cli/docs")}\n`).action(async (queryParts) => {
