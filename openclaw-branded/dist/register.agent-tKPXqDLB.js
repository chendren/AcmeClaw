...
async function agentViaGatewayCommand(opts, runtime) {
	...
	if (agentId) {
		if (!listAgentIds(cfg).includes(agentId)) throw new Error(`Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("acmeclaw agents list")}" to see configured agents.`);
	}
...
async function agentsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	try {
		await prompter.intro("Add AcmeClaw agent");
		...
		} else await prompter.note(["Routing unchanged. Add bindings when you're ready.", "Docs: https://docs.acmeclaw.ai/concepts/multi-agent"].join("\n"), "Routing");
...
async function agentsListCommand(opts, runtime = defaultRuntime) {
	...
	lines.push(`Channel status reflects local config/creds. For live health: ${formatCliCommand("acmeclaw channels status --probe")}.`);
...
${formatHelpExamples([
		["acmeclaw agent --to +15555550123 --message \"status update\"", "Start a new session."],
		["acmeclaw agent --agent ops --message \"Summarize logs\"", "Use a specific agent."],
		["acmeclaw agent --session-id 1234 --message \"Summarize inbox\" --thinking medium", "Target a session with explicit thinking level."],
		["acmeclaw agent --to +15555550123 --message \"Trace logs\" --verbose on --json", "Enable verbose logging and JSON output."],
		["acmeclaw agent --to +15555550123 --message \"Summon reply\" --deliver", "Deliver reply."],
		["acmeclaw agent --agent ops --message \"Generate report\" --deliver --reply-channel slack --reply-to \"#reports\"", "Send reply to a different channel/target."]
	])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.acmeclaw.ai/cli/agent")}`).action(async (opts) => {
...
${theme.heading("Examples:")}
${formatHelpExamples([
		["acmeclaw agents set-identity --agent main --name \"AcmeClaw\" --emoji \"🦞\"", "Set name + emoji."],
		["acmeclaw agents set-identity --agent main --avatar avatars/acmeclaw.png", "Set avatar path."],
		["acmeclaw agents set-identity --workspace ~/.acmeclaw/workspace --from-identity", "Load from IDENTITY.md."],
		["acmeclaw agents set-identity --identity-file ~/.acmeclaw/workspace/IDENTITY.md --agent main", "Use a specific IDENTITY.md."]
	])}
`).action(async (opts) => {
		await runCommandWithRuntime(defaultRuntime, async () => {
