...
function resolveCommandEnv(params) {
	...
	return markAcmeClawExecEnv(resolvedEnv);
}
...
//#endregion
//#region src/infra/acmeclaw-root.ts
const CORE_PACKAGE_NAMES = new Set(["acmeclaw"]);
...
async function resolveAcmeClawPackageRoot(opts) {
...
function resolveAcmeClawPackageRootSync(opts) {
	for (const candidate of buildCandidates(opts)) {
...
async function resolveWorkspaceTemplateDir(opts) {
	...
	resolvingTemplateDir = (async () => {
		...
		const packageRoot = await resolveAcmeClawPackageRoot({
			moduleUrl,
			argv1,
			cwd
		});
...
function resolveDefaultAgentWorkspaceDir(env = process.env, homedir = os.homedir) {
	...
	if (profile && profile.toLowerCase() !== "default") return path.join(home, ".acmeclaw", `workspace-${profile}`);
	return path.join(home, ".acmeclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".acmeclaw";
...
const warningFilterKey = Symbol.for("acmeclaw.warning-filter");
