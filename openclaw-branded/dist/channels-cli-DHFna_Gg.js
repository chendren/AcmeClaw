...
function formatGatewayChannelsStatusLines(payload) {
	...
	if (issues.length > 0) {
		...
		lines.push(`- Run: ${formatCliCommand("acmeclaw doctor")}`);
		lines.push("");
	}
...
function registerChannelsCli(program) {
	...
	const channels = program.command("channels").description("Manage connected chat channels and accounts").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["acmeclaw channels list", "List configured channels and auth profiles."],
		["acmeclaw channels status --probe", "Run channel status checks and probes."],
		["acmeclaw channels add --channel telegram --token <token>", "Add or update a channel account non-interactively."],
		["acmeclaw channels login --channel whatsapp", "Link a WhatsApp Web account."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/channels", "docs.acmeclaw.ai/cli/channels")}\n`);
