...
async function collectChannelSecurityFindings(params) {
	...
	const warnDmPolicy = async (input) => {
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + formatCliCommand("acmeclaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
		...
		for (const accountId of orderedAccountIds) {
			...
			if (plugin.id === "discord") {
				...
				addDiscordNameBasedEntries({
					...
					source: "~/.acmeclaw/credentials/discord-allowFrom.json"
				});
...
function extractHookToken(req) {
	...
	const headerToken = typeof req.headers["x-acmeclaw-token"] === "string" ? req.headers["x-acmeclaw-token"].trim() : "";
...
function resolveHookIdempotencyKey(params) {
	return resolveOptionalHookIdempotencyKey(params.headers?.["idempotency-key"]) || resolveOptionalHookIdempotencyKey(params.headers?.["x-acmeclaw-idempotency-key"]) || resolveOptionalHookIdempotencyKey(params.payload.idempotencyKey);
}
...
function collectSyncedFolderFindings(params) {
	...
	if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) findings.push({
		...
		remediation: `Keep OPENCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("acmeclaw security audit --fix")}".`
	});
	return findings;
}
...
function collectHooksHardeningFindings(cfg, env = process.env) {
	...
	const acmeclawGatewayToken = typeof env.OPENCLAW_GATEWAY_TOKEN === "string" && env.OPENCLAW_GATEWAY_TOKEN.trim() ? env.OPENCLAW_GATEWAY_TOKEN.trim() : null;
	const gatewayToken = gatewayAuth.mode === "token" && typeof gatewayAuth.token === "string" && gatewayAuth.token.trim() ? gatewayAuth.token.trim() : acmeclawGatewayToken ? acmeclawGatewayToken : null;
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-acmeclaw-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
...
function collectSandboxDangerousConfigFindings(cfg) {
	...
	if (browserExposurePaths.length > 0) findings.push({
		...
		remediation: "Set sandbox.browser.network to a dedicated bridge network (recommended default: acmeclaw-sandbox-browser), or set sandbox.browser.cdpSourceRange (for example 172.21.0.1/32) to restrict container-edge CDP ingress."
	});
	return findings;
}
...
function collectLikelyMultiUserSetupFindings(cfg) {
	...
	findings.push({
		...
		detail: "Heuristic signals indicate this gateway may be reachable by multiple users:\n" + signals.map((signal) => `- ${signal}`).join("\n") + `\n${impactLine}\n${riskyContextsDetail}\nAcmeClaw's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.`,
...
async function listSandboxBrowserContainers(execDockerRawFn) {
	try {
		const result = await execDockerRawFn([
			...
			"-a",
			"--filter",
			"label=acmeclaw.sandboxBrowser=1",
			"--format",
			"{{.Names}}"
		], { allowFailure: true });
...
async function readSandboxBrowserHashLabels(params) {
	try {
		const result = await params.execDockerRawFn([
			"inspect",
			"-f",
			"{{ index .Config.Labels \"acmeclaw.configHash\" }}	{{ index .Config.Labels \"acmeclaw.browserConfigEpoch\" }}",
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (missingHash.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("acmeclaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (staleEpoch.length > 0) findings.push({
		...
		detail: `Containers: ${staleEpoch.join(", ")}. Expected acmeclaw.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
		remediation: `${formatCliCommand("acmeclaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("acmeclaw sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	return findings;
}
async function collectPluginsTrustFindings(params) {
	...
	if (pluginDirs.length > 0) {
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = isToolAllowedByPolicies("__acmeclaw_plugin_probe__", resolveToolPolicies({
					cfg: params.cfg,
					agentTools: context.tools,
					sandboxMode,
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (pluginVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `acmeclaw plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (hookVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `acmeclaw hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
...
async function collectPluginsCodeSafetyFindings(params) {
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `acmeclaw security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		if (escapedEntries.length > 0) findings.push({
			...
			remediation: "Update the plugin manifest so all acmeclaw.extensions entries stay inside the plugin directory."
		});
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your AcmeClaw extensions state directory."
			});
...
async function collectInstalledSkillsCodeSafetyFindings(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		for (const entry of entries) {
			if (entry.skill.source === "acmeclaw-bundled") continue;
			...
			const summary = await getCodeSafetySummary({
				...
				findings.push({
					...
					remediation: "Check file permissions and skill layout, then rerun `acmeclaw security audit --deep`."
				});
				return null;
			});
			if (!summary) continue;
...
async function collectFilesystemFindings(params) {
	...
	if (stateDirPerms.ok) {
		...
		if (stateDirPerms.worldWritable) findings.push({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your AcmeClaw state.`,
			remediation: formatPermissionRemediation({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your AcmeClaw state.`,
			remediation: formatPermissionRemediation({
				targetPath: params.stateDir,
				perms: stateDirPerms,
				isDir: true,
...
function collectBrowserControlFindings(cfg, env) {
	...
	try {
		...
		findings.push({
			...
			detail: String(err),
			remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("acmeclaw security audit --deep")}".`
		});
		return findings;
	}
...
async function runSecurityAudit(opts) {
	...
	if (deep?.gateway?.attempted && !deep.gateway.ok) findings.push({
		...
		remediation: `Run "${formatCliCommand("acmeclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("acmeclaw security audit --deep")}".`
	});
	if (deepProbeResult?.authWarning) findings.push({
		...
		remediation: `Set OPENCLAW_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("acmeclaw security audit --deep")}".`
	});
