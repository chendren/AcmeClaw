...
async function setupInternalHooks(cfg, runtime, prompter) {
	await prompter.note([
		...
		"",
		"Learn more: https://docs.acmeclaw.ai/automation/hooks"
	].join("\n"), "Hooks");
	...
	await prompter.note([
		...
		`  ${formatCliCommand("acmeclaw hooks list")}`,
		`  ${formatCliCommand("acmeclaw hooks enable <name>")}`,
		`  ${formatCliCommand("acmeclaw hooks disable <name>")}`
	].join("\n"), "Hooks Configured");
	return next;
}
//#endregion
