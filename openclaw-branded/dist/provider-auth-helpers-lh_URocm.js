...
function resolveKnownAgentId(params) {
	...
	if (!listAgentIds(params.cfg).includes(agentId)) throw new Error(`Unknown agent id "${raw}". Use "${formatCliCommand("acmeclaw agents list")}" to see configured agents.`);
	return agentId;
}
...
const resolveAuthAgentDir = (agentDir) => agentDir ?? resolveAcmeClawAgentDir();
