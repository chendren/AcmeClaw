...
async function finalizeOnboardingWizard(options) {
	...
	if (!opts.skipHealth) {
		...
		try {
			...
			await prompter.note([
				"Docs:",
				"https://docs.acmeclaw.ai/gateway/health",
				"https://docs.acmeclaw.ai/gateway/troubleshooting"
			].join("\n"), "Health check help");
		}
	}
	...
	await prompter.note([
		...
		gatewayStatusLine,
		"Docs: https://docs.acmeclaw.ai/web/control-ui"
	].filter(Boolean).join("\n"), "Control UI");
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		await prompter.note([
			...
			"Stored in: ~/.acmeclaw/acmeclaw.json (gateway.auth.token) or OPENCLAW_GATEWAY_TOKEN.",
			`View token: ${formatCliCommand("acmeclaw config get gateway.auth.token")}`,
			`Generate token: ${formatCliCommand("acmeclaw doctor --generate-gateway-token")}`,
			"Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
			`Open the dashboard anytime: ${formatCliCommand("acmeclaw dashboard --no-open")}`,
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control AcmeClaw." : "Copy/paste this URL in a browser on this machine to control AcmeClaw.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("acmeclaw dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.acmeclaw.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.acmeclaw.ai/security", "Security");
	...
	if (webSearchProvider) {
		...
		if (webSearchEnabled !== false && hasKey) await prompter.note([
			...
			`Provider: ${label}`,
			...keySource ? [keySource] : [],
			"Docs: https://docs.acmeclaw.ai/tools/web"
		].join("\n"), "Web search");
		else if (!hasKey) await prompter.note([
			`Provider ${label} is selected but no API key was found.`,
			"web_search will not work until a key is added.",
			`  ${formatCliCommand("acmeclaw configure --section web")}`,
			"",
			`Get your key at: ${entry?.signupUrl ?? "https://docs.acmeclaw.ai/tools/web"}`,
			"Docs: https://docs.acmeclaw.ai/tools/web"
		].join("\n"), "Web search");
		else await prompter.note([
			`Web search (${label}) is configured but disabled.`,
			`Re-enable: ${formatCliCommand("acmeclaw configure --section web")}`,
			"",
			"Docs: https://docs.acmeclaw.ai/tools/web"
		].join("\n"), "Web search");
	} else {
		...
		if (legacyDetected) await prompter.note([`Web search is available via ${legacyDetected.label} (auto-detected).`, "Docs: https://docs.acmeclaw.ai/tools/web"].join("\n"), "Web search");
		else await prompter.note([
	...
	await prompter.note("What now: https://acmeclaw.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "Onboarding complete. Dashboard opened; keep that tab to control AcmeClaw." : "Onboarding complete. Use the dashboard link above to control AcmeClaw.");
	return { launchedTui };
}
//#endregion
