...
function configureCandidateSortKey(candidate) {
	...
	return `acmeclaw:${candidate.path}`;
}
...
function buildConfigureCandidatesForScope(params) {
	const authoredConfig = params.authoredAcmeClawConfig ?? params.config;
	const hasPathInAuthoredConfig = (pathSegments) => hasPath(authoredConfig, pathSegments);
	const acmeclawCandidates = discoverConfigSecretTargets(params.config).filter((entry) => entry.entry.includeInConfigure).map((entry) => {
	...
	return [...acmeclawCandidates, ...authCandidates].toSorted((a, b) => configureCandidateSortKey(a).localeCompare(configureCandidateSortKey(b)));
}
...
function buildSecretsConfigurePlan(params) {
	return {
		...
		generatedBy: "acmeclaw secrets configure",
...
async function runSecretsConfigureInteractive(params = {}) {
	...
	if (!params.providersOnly) {
		...
		const candidates = buildConfigureCandidatesForScope({
			config: stagedConfig,
			authoredAcmeClawConfig: snapshot.resolved,
			authProfiles: {
				agentId: configureAgentId,
				store: authStore
			}
		...
		while (true) {
			...
			const options = visibleCandidates.map((candidate) => ({
				...
				hint: [candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "acmeclaw.json", candidate.isDerived === true ? "derived" : void 0].filter(Boolean).join(" | ")
			}));
			options.push({
