...
const GATEWAY_LAUNCH_AGENT_LABEL = "ai.acmeclaw.gateway";
const GATEWAY_SYSTEMD_SERVICE_NAME = "acmeclaw-gateway";
const GATEWAY_WINDOWS_TASK_NAME = "AcmeClaw Gateway";
const GATEWAY_SERVICE_MARKER = "acmeclaw";
const GATEWAY_SERVICE_KIND = "gateway";
const NODE_LAUNCH_AGENT_LABEL = "ai.acmeclaw.node";
const NODE_SYSTEMD_SERVICE_NAME = "acmeclaw-node";
const NODE_WINDOWS_TASK_NAME = "AcmeClaw Node";
const NODE_SERVICE_MARKER = "acmeclaw";
...
function formatGatewayServiceDescription(params) {
	...
	if (parts.length === 0) return "AcmeClaw Gateway";
	return `AcmeClaw Gateway (${parts.join(", ")})`;
}
...
function formatNodeServiceDescription(params) {
	const version = params?.version?.trim();
	if (!version) return "AcmeClaw Node Host";
	return `AcmeClaw Node Host (v${version})`;
}
//#endregion
