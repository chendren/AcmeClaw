...
import { a as openBoundaryFileSync, f as isNotFoundPathError, g as matchesSkillFilter, i as openBoundaryFile, o as openVerifiedFileSync, s as sameFileIdentity, t as resolveAcmeClawPackageRoot } from "./acmeclaw-root-3m-COvjr.js";
...
import { t as ensureAcmeClawCliOnPath } from "./path-env-Ce_2-thI.js";
...
async function runAutoUpdateCommand(params) {
	...
	if (argv.length === 0) argv.push("acmeclaw", ...baseArgs);
	try {
...
async function runGatewayUpdateCheck(params) {
	...
	const root = await resolveAcmeClawPackageRoot({
	...
	if (cmp != null && cmp < 0) {
		...
		if (shouldRunUpdateHints && shouldNotify) {
			params.log.info(`update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("acmeclaw update")}`);
...
function safeServiceName(name) {
	...
	return trimmed.length > 0 ? trimmed : "AcmeClaw";
}
function prettifyInstanceName(name) {
	const normalized = name.trim().replace(/\s+/g, " ");
	return normalized.replace(/\s+\(AcmeClaw\)\s*$/i, "").trim() || normalized;
}
...
async function startGatewayBonjourAdvertiser(opts) {
	...
	const hostname = (process.env.OPENCLAW_MDNS_HOSTNAME?.trim() || process.env.CLAWDBOT_MDNS_HOSTNAME?.trim() || "acmeclaw").replace(/\.local$/i, "").split(".")[0].trim() || "acmeclaw";
	const instanceName = typeof opts.instanceName === "string" && opts.instanceName.trim() ? opts.instanceName.trim() : `${hostname} (AcmeClaw)`;
...
function formatBonjourInstanceName(displayName) {
	...
	if (!trimmed) return "AcmeClaw";
	if (/acmeclaw/i.test(trimmed)) return trimmed;
	return `${trimmed} (AcmeClaw)`;
}
function resolveBonjourCliPath(opts = {}) {
	...
	const siblingCli = path.join(execDir, "acmeclaw");
	...
	const binCli = path.join(cwd, "bin", "acmeclaw");
...
const PLUGIN_RUNTIME_GATEWAY_REQUEST_SCOPE_KEY = Symbol.for("acmeclaw.pluginRuntimeGatewayRequestScope");
...
/**
* Build TimestampInjectionOptions from an AcmeClawConfig.
*/
...
function appendInjectedAssistantMessageToTranscript(params) {
	...
	const messageBody = {
		...
		...params.abortMeta ? { acmeclawAbort: {
			aborted: true,
...
function buildBaseConfigSchema() {
	...
	const schema = AcmeClawSchema.toJSONSchema({
		target: "draft-07",
		unrepresentable: "any"
	});
	schema.title = "AcmeClawConfig";
	const hints = applyDerivedTags(mapSensitivePaths(AcmeClawSchema, "", buildBaseHints()));
	const next = {
...
const MAX_LIMIT = 5e3;
const MAX_BYTES = 1e6;
const ROLLING_LOG_RE = /^acmeclaw-\d{4}-\d{2}-\d{2}\.log$/;
...
const CANVAS_CAPABILITY_PATH_PREFIX = "/__acmeclaw__/cap";
	...
	try {
		...
		if (registration.transport === "relay") {
			...
			result = await sendApnsAlert({
				registration,
				nodeId,
				title: "AcmeClaw needs a quick reopen",
				body: "Tap to reopen AcmeClaw and restore the node connection.",
				relayConfig: relay.relayConfig
			});
		} else {
			const auth = await resolveDirectNodePushConfig();
...
const pushHandlers = { "push.test": async ({ params, respond }) => {
	...
	const title = normalizeOptionalString(params.title) ?? "AcmeClaw";
...
const updateHandlers = { "update.run": async ({ params, respond, client, context }) => {
	...
	try {
		...
		result = await runGatewayUpdate({
			timeoutMs,
			cwd: await resolveAcmeClawPackageRoot({
				moduleUrl: import.meta.url,
				argv1: process.argv[1],
				cwd: process.cwd()
			}) ?? process.cwd(),
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY = Symbol.for("acmeclaw.fallbackGatewayContextState");
...
function loadGatewayPlugins(params) {
	const pluginRegistry = loadAcmeClawPlugins({
		config: params.cfg,
		workspaceDir: params.workspaceDir,
		logger: {
...
const A2UI_PATH = "/__acmeclaw__/a2ui";
const CANVAS_HOST_PATH = "/__acmeclaw__/canvas";
const CANVAS_WS_PATH = "/__acmeclaw__/ws";
...
(() => {
  ...
  // Works on:
  // - iOS: window.webkit.messageHandlers.acmeclawCanvasA2UIAction.postMessage(...)
  // - Android: window.acmeclawCanvasA2UIAction.postMessage(...)
  const handlerNames = ["acmeclawCanvasA2UIAction"];
  ...
  globalThis.AcmeClaw = globalThis.AcmeClaw ?? {};
  globalThis.AcmeClaw.postMessage = postToNode;
  globalThis.AcmeClaw.sendUserAction = sendUserAction;
  globalThis.acmeclawPostMessage = postToNode;
  globalThis.acmeclawSendUserAction = sendUserAction;
...
async function handleA2uiHttpRequest(req, res) {
	...
	const basePath = url.pathname === "/__acmeclaw__/a2ui" || url.pathname.startsWith(`/__acmeclaw__/a2ui/`) ? A2UI_PATH : void 0;
...
function handleControlUiHttpRequest(req, res, opts) {
	...
	applyControlUiSecurityHeaders(res);
	if (pathname === (basePath ? `${basePath}/__acmeclaw/control-ui-config.json` : "/__acmeclaw/control-ui-config.json")) {
...
function resolveAgentIdFromHeader(req) {
	const raw = getHeader(req, "x-acmeclaw-agent-id")?.trim() || getHeader(req, "x-acmeclaw-agent")?.trim() || "";
	if (!raw) return;
...
function resolveAgentIdFromModel(model) {
	...
	if (!raw) return;
	const agentId = (raw.match(/^acmeclaw[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ?? raw.match(/^agent:(?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i))?.groups?.agentId;
	if (!agentId) return;
...
function resolveSessionKey(params) {
	const explicit = getHeader(params.req, "x-acmeclaw-session-key")?.trim();
...
function resolveGatewayRequestContext(params) {
	...
	return {
		...
		messageChannel: params.useMessageChannelHeader ? normalizeMessageChannel(getHeader(params.req, "x-acmeclaw-message-channel")) ?? params.defaultMessageChannel : params.defaultMessageChannel
	};
}
//#endregion
...
function resolveAgentResponseText(result) {
	...
	if (!Array.isArray(payloads) || payloads.length === 0) return "No response from AcmeClaw.";
	return payloads.map((p) => typeof p.text === "string" ? p.text : "").filter(Boolean).join("\n\n") || "No response from AcmeClaw.";
}
async function handleOpenAiHttpRequest(req, res, opts) {
	...
	const model = typeof payload.model === "string" ? payload.model : "acmeclaw";
...
*
* Implements the OpenResponses `/v1/responses` endpoint for AcmeClaw Gateway.
*
* @see https://www.open-responses.com/
*/
...
async function handleOpenResponsesHttpRequest(req, res, opts) {
	...
	unsubscribe = onAgentEvent((evt) => {
		...
		if (evt.stream === "lifecycle") {
			const phase = evt.data?.phase;
			if (phase === "end" || phase === "error") requestFinalize(phase === "error" ? "failed" : "completed", accumulatedText || "No response from AcmeClaw.");
		}
	});
	req.on("close", () => {
		closed = true;
...
function isCanvasPath(pathname) {
	return pathname === "/__acmeclaw__/a2ui" || pathname.startsWith(`/__acmeclaw__/a2ui/`) || pathname === "/__acmeclaw__/canvas" || pathname.startsWith(`/__acmeclaw__/canvas/`) || pathname === "/__acmeclaw__/ws";
}
...
async function handleToolsInvokeHttpRequest(req, res, opts) {
	...
	const messageChannel = normalizeMessageChannel(getHeader(req, "x-acmeclaw-message-channel") ?? "");
	const accountId = getHeader(req, "x-acmeclaw-account-id")?.trim() || void 0;
	const agentTo = getHeader(req, "x-acmeclaw-message-to")?.trim() || void 0;
	const agentThreadId = getHeader(req, "x-acmeclaw-thread-id")?.trim() || void 0;
...
function createHooksRequestHandler(opts) {
	...
	return async (req, res) => {
		...
		if (url.searchParams.has("token")) {
			res.statusCode = 400;
			res.setHeader("Content-Type", "text/plain; charset=utf-8");
			res.end("Hook token must be provided via Authorization: Bearer <token> or X-AcmeClaw-Token header (query parameters are not allowed).");
			return true;
		}
		if (req.method !== "POST") {
			res.statusCode = 405;
...
function attachGatewayUpgradeHandler(opts) {
	...
	httpServer.on("upgrade", (req, socket, head) => {
		(async () => {
			...
			if (scopedCanvas.rewrittenUrl) req.url = scopedCanvas.rewrittenUrl;
			if (canvasHost) {
				if (new URL(req.url ?? "/", "http://localhost").pathname === "/__acmeclaw__/ws") {
					const configSnapshot = loadConfig();
					const ok = await authorizeCanvasRequest({
						req,
						auth: resolvedAuth,
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.acmeclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.acmeclawCanvasA2UIAction &&
        typeof window.acmeclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.acmeclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("acmeclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AcmeClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.acmeclawSendUserAction === "function"
        ? window.acmeclawSendUserAction
        : undefined;
...
function normalizeBasePath(rawPath) {
	const normalized = normalizeUrlPath((rawPath ?? "/__acmeclaw__/canvas").trim() || "/__acmeclaw__/canvas");
...
async function createCanvasHostHandler(opts) {
	...
	const handleUpgrade = (req, socket, head) => {
		if (!wss) return false;
		if (new URL(req.url ?? "/", "http://localhost").pathname !== "/__acmeclaw__/ws") return false;
		wss.handleUpgrade(req, socket, head, (ws) => {
			wss.emit("connection", ws, req);
		});
		return true;
	...
	const handleHttpRequest = async (req, res) => {
		...
		try {
			const url = new URL(urlRaw, "http://localhost");
			if (url.pathname === "/__acmeclaw__/ws") {
			...
			if (!opened) {
				if (urlPath === "/" || urlPath.endsWith("/")) {
					res.statusCode = 404;
					res.setHeader("Content-Type", "text/html; charset=utf-8");
					res.end(`<!doctype html><meta charset="utf-8" /><title>AcmeClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`);
					return true;
				}
				res.statusCode = 404;
				res.setHeader("Content-Type", "text/plain; charset=utf-8");
...
function logGatewayStartup(params) {
	...
	if (enabledDangerousFlags.length > 0) {
		const warning = `security warning: dangerous config flags enabled: ${enabledDangerousFlags.join(", ")}. Run \`acmeclaw security audit\`.`;
...
*/
const IMMUTABLE_SOURCES = new Set(["acmeclaw-bundled"]);
...
function maybeWarnTrustedHookSource(source) {
	if (source === "acmeclaw-workspace") {
		log$2.warn("Loading workspace hook code into the gateway process. Workspace hooks are trusted local code.");
		return;
	}
	if (source === "acmeclaw-managed") log$2.warn("Loading managed hook code into the gateway process. Managed hooks are trusted local code.");
}
/**
...
*
* @param cfg - AcmeClaw configuration
...
async function loadInternalHooks(cfg, workspaceDir, opts) {
	...
	for (const handlerConfig of handlers) try {
		...
		const mod = await import(buildImportUrl(safeModulePath, "acmeclaw-workspace"));
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	if (configSnapshot.exists && !configSnapshot.valid) {
		...
		throw new Error(`Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("acmeclaw doctor")}" to repair, then retry.`);
	}
	...
	else log.warn("Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `acmeclaw config set gateway.auth.mode token` and `acmeclaw config set gateway.auth.token <token>`.");
...
async function ensureDevWorkspace(dir) {
	...
	const [agents, soul, tools, identity, user] = await Promise.all([
		loadDevTemplate("AGENTS.dev.md", `# AGENTS.md - AcmeClaw Dev Workspace\n\nDefault dev workspace for acmeclaw gateway --dev.\n`),
...
function restartGatewayProcessWithFreshPid() {
	...
	if (supervisor) {
		...
		if (supervisor === "schtasks") {
			const restart = triggerAcmeClawRestart();
...
async function runGatewayCommand$1(opts) {
	...
	if (!opts.allowUnconfigured && mode !== "local") {
		if (!configExists) defaultRuntime.error(`Missing config. Run \`${formatCliCommand("acmeclaw setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`);
		else {
	...
	try {
		...
		if (err instanceof GatewayLockError || err && typeof err === "object" && err.name === "GatewayLockError") {
			...
			defaultRuntime.error(`Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("acmeclaw gateway stop")}`);
			try {
...
function registerGatewayCli(program) {
	const gateway = addGatewayRunCommand(program.command("gateway").description("Run, inspect, and query the WebSocket Gateway").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["acmeclaw gateway run", "Run the gateway in the foreground."],
		["acmeclaw gateway status", "Show service status and probe reachability."],
		["acmeclaw gateway discover", "Find local and wide-area gateway beacons."],
		["acmeclaw gateway call health", "Call a gateway RPC method directly."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.acmeclaw.ai/cli/gateway")}\n`));
