...
async function createWaSocket(printQr, verbose, opts = {}) {
	...
	sock.ev.on("connection.update", (update) => {
		try {
			...
			if (connection === "close") {
				if (getStatusCode(lastDisconnect?.error) === DisconnectReason.loggedOut) console.error(danger(`WhatsApp session logged out. Run: ${formatCliCommand("acmeclaw channels login")}`));
			}
