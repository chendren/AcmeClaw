...
async function downloadFile(params) {
	...
	const stagingDir = path.join(params.rootDir, ".acmeclaw-download-staging");
...
async function collectSkillInstallScanWarnings(entry) {
	...
	try {
		...
		} else if (summary.warn > 0) warnings.push(`Skill "${skillName}" has ${summary.warn} suspicious code pattern(s). Run "acmeclaw security audit --deep" for details.`);
	} catch (err) {
		warnings.push(`Skill "${skillName}" code safety scan failed (${String(err)}). Installation continues; run "acmeclaw security audit --deep" after install.`);
	}
	return warnings;
}
