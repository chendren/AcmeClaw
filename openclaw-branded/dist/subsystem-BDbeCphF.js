...
function buildParseArgv(params) {
	...
	const normalizedArgv = programName && baseArgv[0] === programName ? baseArgv.slice(1) : baseArgv[0]?.endsWith("acmeclaw") ? baseArgv.slice(1) : baseArgv;
	if (normalizedArgv.length >= 2 && (isNodeRuntime(normalizedArgv[0] ?? "") || isBunRuntime(normalizedArgv[0] ?? ""))) return normalizedArgv;
	return [
		"node",
		programName || "acmeclaw",
		...normalizedArgv
	];
}
...
//#endregion
//#region src/infra/tmp-acmeclaw-dir.ts
const POSIX_OPENCLAW_TMP_DIR = "/tmp/acmeclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;
function isNodeErrorWithCode(err, code) {
	return typeof err === "object" && err !== null && "code" in err && err.code === code;
}
function resolvePreferredAcmeClawTmpDir(options = {}) {
	...
	const fallback = () => {
		const base = tmpdir();
		const suffix = uid === void 0 ? "acmeclaw" : `acmeclaw-${uid}`;
	...
	const tryRepairWritableBits = (candidatePath) => {
		try {
			...
			warn(`[acmeclaw] tightened permissions on temp dir: ${candidatePath}`);
	...
	const ensureTrustedFallbackDir = () => {
		...
		if (state === "invalid") {
			...
			throw new Error(`Unsafe fallback AcmeClaw temp dir: ${fallbackPath}`);
		}
		try {
			...
			throw new Error(`Unable to create fallback AcmeClaw temp dir: ${fallbackPath}`);
		}
		if (resolveDirState(fallbackPath) !== "available" && !tryRepairWritableBits(fallbackPath)) throw new Error(`Unsafe fallback AcmeClaw temp dir: ${fallbackPath}`);
		return fallbackPath;
	};
	const existingPreferredState = resolveDirState(POSIX_OPENCLAW_TMP_DIR);
	if (existingPreferredState === "available") return POSIX_OPENCLAW_TMP_DIR;
	if (existingPreferredState === "invalid") {
		if (tryRepairWritableBits("/tmp/acmeclaw")) return POSIX_OPENCLAW_TMP_DIR;
	...
	try {
		...
		if (resolveDirState("/tmp/acmeclaw") !== "available" && !tryRepairWritableBits("/tmp/acmeclaw")) return ensureTrustedFallbackDir();
...
function resolveEnvLogLevelOverride() {
	...
	if (loggingState.invalidEnvLogLevelValue !== trimmed) {
		...
		process.stderr.write(`[acmeclaw] Ignoring invalid OPENCLAW_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`);
	}
}
//#endregion
...
const DEFAULT_LOG_DIR = resolvePreferredAcmeClawTmpDir();
path.join(DEFAULT_LOG_DIR, "acmeclaw.log");
const LOG_PREFIX = "acmeclaw";
		...
		try {
			...
			if (nextBytes > settings.maxFileBytes) {
				if (!warnedAboutSizeCap) {
					...
					process.stderr.write(`[acmeclaw] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`);
				}
				return;
			}
