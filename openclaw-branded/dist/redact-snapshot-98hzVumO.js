...
const FIELD_HELP = {
	meta: "Metadata fields automatically maintained by AcmeClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
	"meta.lastTouchedVersion": "Auto-set when AcmeClaw writes the config.",
	...
	"env.vars": "Explicit key/value environment variable overrides merged into runtime process environment for AcmeClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
	...
	"wizard.lastRunVersion": "AcmeClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version onboarding changes.",
	...
	update: "Update-channel and startup-check behavior for keeping AcmeClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
	...
	"agents.list[].runtime": "Optional runtime descriptor for this agent. Use embedded for default AcmeClaw execution or acp for external ACP harness defaults.",
	"agents.list[].runtime.type": "Runtime type for this agent: \"embedded\" (default AcmeClaw runtime) or \"acp\" (ACP harness defaults).",
	...
	"agents.list[].runtime.acp.agent": "Optional ACP harness agent id to use for this AcmeClaw agent (for example codex, claude).",
	...
	"browser.profiles.*.driver": "Per-profile browser driver mode: \"acmeclaw\" (or legacy \"clawd\") or \"extension\" depending on connection/runtime strategy. Use the driver that matches your browser control stack to avoid protocol mismatches.",
	...
	"discovery.wideArea.domain": "Optional unicast DNS-SD domain for wide-area discovery, such as acmeclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
	...
	"agents.defaults.sandbox.browser.network": "Docker network for sandbox browser containers (default: acmeclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
	...
	"gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /acmeclaw).",
	...
	"gateway.nodes.allowCommands": "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `acmeclaw security audit`.",
	...
	"models.providers.*.injectNumCtxForOpenAICompat": "Controls whether AcmeClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
	...
	"agents.defaults.memorySearch.store.path": "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.acmeclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
	...
	"memory.backend": "Selects the global memory engine: \"builtin\" uses AcmeClaw memory internals, while \"qmd\" uses the QMD sidecar pipeline. Keep \"builtin\" unless you intentionally operate QMD.",
	...
	"plugins.installs": "CLI-managed install metadata (used by `acmeclaw plugins update` to locate install sources).",
	"plugins.installs.*.source": "Install source (\"npm\", \"archive\", or \"path\").",
	"plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
	"plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
	"plugins.installs.*.installPath": "Resolved install directory (usually ~/.acmeclaw/extensions/<id>).",
	...
	"agents.defaults.embeddedPi": "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in AcmeClaw sessions.",
	...
	"session.parentForkMaxTokens": "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, AcmeClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
	...
	hooks: "Inbound webhook automation surface for mapping external events into wake or agent actions in AcmeClaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
	...
	"channels.irc": "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into AcmeClaw.",
	...
	"channels.discord.eventQueue.listenerTimeout": `Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is ${DISCORD_DEFAULT_LISTENER_TIMEOUT_MS} in AcmeClaw; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.`,
...
const FIELD_PLACEHOLDERS = {
	...
	"gateway.controlUi.basePath": "/acmeclaw",
	"gateway.controlUi.root": "dist/control-ui",
	"gateway.controlUi.allowedOrigins": "https://control.example.com",
	"gateway.push.apns.relay.baseUrl": "https://relay.example.com",
	"channels.mattermost.baseUrl": "https://chat.example.com",
	"agents.list[].identity.avatar": "avatars/acmeclaw.png"
};
