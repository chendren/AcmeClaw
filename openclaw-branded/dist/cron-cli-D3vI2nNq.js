...
function registerCronCli(program) {
	const cron = program.command("cron").description("Manage cron jobs (via Gateway)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.acmeclaw.ai/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`acmeclaw doctor --fix\` to normalize legacy cron job storage.\n`);
