...
async function ensureAcmeClawHooks(manifest) {
	const hooks = manifest[MANIFEST_KEY]?.hooks;
	if (!Array.isArray(hooks)) throw new Error("package.json missing acmeclaw.hooks");
	const list = hooks.map((e) => typeof e === "string" ? e.trim() : "").filter(Boolean);
	if (list.length === 0) throw new Error("package.json acmeclaw.hooks is empty");
	return list;
}
...
async function installHookPackageFromDir(params) {
	...
	let hookEntries;
	try {
		hookEntries = await ensureAcmeClawHooks(manifest);
	} catch (err) {
		return {
			ok: false,
			error: String(err)
	...
	for (const entry of hookEntries) {
		...
		if (!isPathInside(params.packageDir, hookDir)) return {
			ok: false,
			error: `acmeclaw.hooks entry escapes package directory: ${entry}`
		};
		await validateHookDir(hookDir);
		if (!isPathInsideWithRealpath(params.packageDir, hookDir, { requireRealpath: true })) return {
			ok: false,
			error: `acmeclaw.hooks entry resolves outside package directory: ${entry}`
		};
...
async function installHooksFromNpmSpec(params) {
	...
	return await installFromValidatedNpmSpecArchive({
		tempDirPrefix: "acmeclaw-hook-pack-",
		spec,
		timeoutMs,
		expectedIntegrity: params.expectedIntegrity,
		onIntegrityDrift: params.onIntegrityDrift,
...
function formatHooksList(report, opts) {
	...
	if (hooks.length === 0) return opts.eligible ? `No eligible hooks found. Run \`${formatCliCommand("acmeclaw hooks list")}\` to see all hooks.` : "No hooks found.";
...
function formatHookInfo(report, hookName, opts) {
	...
	if (!hook) {
		...
		return `Hook "${hookName}" not found. Run \`${formatCliCommand("acmeclaw hooks list")}\` to see available hooks.`;
	}
