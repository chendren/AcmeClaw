...
import { addTranscriptEntry, consumePendingUserTranscript, transitionState } from "./state.js";
...
const DTMF_STREAM_RECONNECT_GRACE_MS = 15_000;

function usesStreamingTranscriptWait(
  ctx: Pick<ConversationContext, "config">,
  provider: NonNullable<ConnectedCallResolution["provider"]>,
): boolean {
  return provider.name === "twilio" && ctx.config.streaming.enabled;
}
...
export async function sendDtmf(
  ...
  const normalizedDigits = normalizeDtmfSequence(digits);
  if (!normalizedDigits) {
    return { success: false, error: "Invalid DTMF sequence" };
  }
  ...
  const reconnectGraceUntil =
    provider.name === "twilio" ? Date.now() + DTMF_STREAM_RECONNECT_GRACE_MS : undefined;
  if (reconnectGraceUntil) {
    call.metadata = {
      ...(call.metadata ?? {}),
      dtmfStreamReconnectExpectedUntil: reconnectGraceUntil,
    };
  ...
  try {
    ...
    addTranscriptEntry(call, "bot", `[DTMF] ${normalizedDigits}`);
    call.metadata = {
      ...(call.metadata ?? {}),
      lastDtmfDigits: normalizedDigits,
      lastDtmfSentAt: Date.now(),
      ...(reconnectGraceUntil ? { dtmfStreamReconnectExpectedUntil: reconnectGraceUntil } : {}),
    };
    persistCallRecord(ctx.storePath, call);
    return { success: true };
  } catch (err) {
    if (reconnectGraceUntil && call.metadata) {
      delete call.metadata.dtmfStreamReconnectExpectedUntil;
      persistCallRecord(ctx.storePath, call);
    }
...
export async function continueCall(
  ...
  const bufferedTranscript = consumePendingUserTranscript(call);
  if (bufferedTranscript) {
    ...
    call.metadata = {
      ...
      turnCount,
      lastTurnLatencyMs: 0,
      lastTurnListenWaitMs: 0,
      lastTurnCompletedAt: completedAt,
      lastTurnAction: "wait",
    };
    persistCallRecord(ctx.storePath, call);
    return { success: true, transcript: bufferedTranscript };
  }
  ctx.activeTurnCalls.add(callId);

  const turnStartedAt = Date.now();
  const useStreamingWait = usesStreamingTranscriptWait(ctx, provider);
  const turnToken =
    provider.name === "twilio" && !useStreamingWait ? crypto.randomUUID() : undefined;
...
export async function waitForPrompt(
  ...
  try {
    ...
    console.log(
      "[voice-call] waitForPrompt latency call=" +
        call.callId +
        " totalMs=" +
        String(lastTurnLatencyMs) +
        " listenWaitMs=" +
