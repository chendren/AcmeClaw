  ...
  private initializeMediaStreaming(): void {
    ...
    const streamConfig: MediaStreamConfig = {
      ...
      onConnect: (callId, streamSid) => {
        ...
        const reconnectedCall = this.manager.getCallByProviderCallId(callId);
        if (reconnectedCall?.metadata) {
          delete reconnectedCall.metadata.dtmfStreamReconnectExpectedUntil;
        }
      ...
      onDisconnect: (callId) => {
        ...
        const reconnectGraceUntil =
          typeof disconnectedCall?.metadata?.dtmfStreamReconnectExpectedUntil === "number"
            ? disconnectedCall.metadata.dtmfStreamReconnectExpectedUntil
            : 0;
        ...
        if (reconnectGraceUntil > Date.now()) {
          console.log(
            `[voice-call] Allowing Twilio stream reconnect for ${disconnectedCall?.callId ?? callId}`,
          );
          return;
        }
  ...
  private processParsedEvents(events: NormalizedEvent[]): void {
    for (const event of events) {
      try {
        ...
        // In non-streaming mode, Twilio/Plivo speech callbacks still arrive as
        // `call.speech` events, but they do not pass through the streaming
        // `onTranscript` hook above. Mirror the same auto-response behavior here
        // so conversation-mode calls can continue hands-free without media streams.
        if (!this.config.streaming.enabled && event.type === "call.speech" && event.isFinal) {
          const call = this.manager.getCall(event.callId);
          const callMode = call?.metadata?.mode as string | undefined;
          const shouldRespond = call && (call.direction === "inbound" || callMode === "conversation");
          if (shouldRespond && event.transcript) {
  ...
  private async handleInboundResponse(callId: string, userMessage: string): Promise<void> {
    ...
    try {
      ...
      if (result.action === "wait") {
        console.log(
          `[voice-call] Auto-response waiting for more context on ${callId}${result.reason ? ` (${result.reason})` : ""}`,
        );
        return;
      }
      ...
      if (result.action === "dtmf") {
        if (!result.digits) {
          console.error("[voice-call] DTMF response missing digits");
          return;
        }
        console.log(`[voice-call] AI response (dtmf): "${result.digits}"`);
        const dtmfResult = await this.manager.sendDtmf(callId, result.digits);
        if (!dtmfResult.success) {
          console.error(`[voice-call] DTMF failed: ${dtmfResult.error ?? "unknown error"}`);
        }
        return;
      }

      if (result.text) {
        console.log(`[voice-call] AI response (${result.action}): "${result.text}"`);
        const speakResult = await this.manager.speak(callId, result.text);
        if (!speakResult.success) {
          console.error(`[voice-call] Speak failed: ${speakResult.error ?? "unknown error"}`);
          return;
        }
      }
      ...
      if (result.action === "hangup") {
        ...
        if (!endResult.success) {
          console.error(`[voice-call] End-call failed: ${endResult.error ?? "unknown error"}`);
        }
      }
    } catch (err) {
