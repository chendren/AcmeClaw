...
function resolveCallMode(mode?: string): "notify" | "conversation" | "manual" | undefined {
  return mode === "notify" || mode === "conversation" || mode === "manual" ? mode : undefined;
}
...
export function registerVoiceCallCli(params: {
  ...
  root
    ...
    .option(
      "--mode <mode>",
      "Call mode: notify (hangup after message), conversation (auto-response), or manual (external control)",
      "conversation",
    )
  ...
  root
    .command("wait")
    .description("Wait for the next IVR prompt without speaking first")
    .requiredOption("--call-id <id>", "Call ID")
    .action(async (options: { callId: string }) => {
      const rt = await ensureRuntime();
      const result = await rt.manager.waitForPrompt(options.callId);
      if (!result.success) {
  ...
  root
    .command("dtmf")
    .description("Send outbound DTMF digits on an active call")
    .requiredOption("--call-id <id>", "Call ID")
    .requiredOption("--digits <digits>", "DTMF digits to send (supports w/W pauses)")
