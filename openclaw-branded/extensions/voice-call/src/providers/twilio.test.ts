...
describe("TwilioProvider", () => {
  ...
  it("updates an active call with Play digits and reconnects the media stream inline", async () => {
    const provider = createProvider();
    provider.parseWebhookEvent(
      createContext("CallStatus=in-progress&Direction=outbound-api&CallSid=CA901", {
        callId: "call-901",
      }),
    );
    ...
    }).apiRequest = async (_endpoint, params) => {
      capturedTwiml = String(params.Twiml ?? "");
      return {};
    };

    await provider.sendDtmf({
      callId: "call-901",
      providerCallId: "CA901",
      digits: "ww123#",
    });

    expect(capturedTwiml).toContain('<Play digits="ww123#"></Play>');
    expect(capturedTwiml).toContain("<Connect>");
    expect(capturedTwiml).toContain('<Stream url="wss://example.ngrok.app/voice/stream">');
    expect(capturedTwiml).toContain('<Parameter name="token" value="');
  });

  it("clears inbound active-stream state before redirecting for DTMF playback", async () => {
    const provider = createProvider();
    provider.parseWebhookEvent(createContext("CallStatus=ringing&Direction=inbound&CallSid=CA902"));
    ...
    await provider.sendDtmf({
      callId: "call-902",
      providerCallId: "CA902",
      digits: "1",
    });

    const reconnectResult = provider.parseWebhookEvent(
      createContext("CallStatus=in-progress&Direction=inbound&CallSid=CA902"),
    );

    expect(reconnectResult.providerResponseBody).toContain("<Connect>");
    expect(reconnectResult.providerResponseBody).not.toContain("hold-queue");
  });
});
