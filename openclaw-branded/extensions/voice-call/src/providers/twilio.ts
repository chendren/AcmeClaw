  ...
  parseWebhookEvent(
    ...
    try {
      ...
      const isStatusCallback =
        typeof ctx.query?.type === "string" && ctx.query.type.trim() === "status";
      const callSid = params.get("CallSid") || "";
      if (callSid && !isStatusCallback) {
        this.callWebhookUrls.set(callSid, ctx.url);
      }
...
<Response>
  ${this.getStreamConnectVerbXml(streamUrl)}
</Response>`;
  }

  private getStreamConnectVerbXml(streamUrl: string): string {
...
<Response>
  ...
  async sendDtmf(input: SendDtmfInput): Promise<void> {
    ...
    // Switching the call to <Play digits> temporarily hands control back to TwiML,
    // so clear stream routing state before we ask Twilio to reconnect afterward.
    this.clearTtsQueue(input.providerCallId);
    this.callStreamMap.delete(input.providerCallId);
    this.activeStreamCalls.delete(input.providerCallId);

    const streamUrl = this.getStreamUrlForCall(input.providerCallId);
    const reconnectVerb = streamUrl
      ? this.getStreamConnectVerbXml(streamUrl)
      : '<Pause length="30"/>';
    const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Play digits="${escapeXml(input.digits)}"></Play>
  ${reconnectVerb}
</Response>`;
