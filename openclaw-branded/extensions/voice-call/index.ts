...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    mode: Type.Optional(
      Type.Union([Type.Literal("notify"), Type.Literal("conversation"), Type.Literal("manual")]),
    ),
  }),
  Type.Object({
  ...
  Type.Object({
    action: Type.Literal("wait_for_prompt"),
    callId: Type.String({ description: "Call ID" }),
  }),
  Type.Object({
    action: Type.Literal("send_dtmf"),
    callId: Type.String({ description: "Call ID" }),
    digits: Type.String({ description: "DTMF digits to send (supports w/W pauses)" }),
  }),
  Type.Object({
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerGatewayMethod(
      ...
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          ...
          const rt = await ensureRuntime();
          const result = await rt.manager.waitForPrompt(callId);
          if (!result.success) {
            respond(false, { error: result.error || "wait failed" });
            return;
          }
    ...
    api.registerGatewayMethod(
      ...
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          const callId = typeof params?.callId === "string" ? params.callId.trim() : "";
          const digits = typeof params?.digits === "string" ? params.digits.trim() : "";
          if (!callId || !digits) {
          ...
          if (typeof params?.action === "string") {
            switch (params.action) {
              case "initiate_call": {
                ...
                const mode =
                  params.mode === "notify" || params.mode === "conversation" || params.mode === "manual"
                    ? params.mode
                    : undefined;
                if (!message && mode !== "manual") {
